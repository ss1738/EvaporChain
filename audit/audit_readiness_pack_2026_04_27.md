...
| K-11 | Validator delegation feature: 7 of 8 phases complete | **In progress 2026-04-27/28.** P0 #4 shipped end-to-end across types, state, execution, consensus, wallet, node, and CLI surfaces. **Done:** Phase 1 types (`a62debb`), Phase 2 exhaustiveness arms across the workspace, Phase 3 StateDB persistence (`aa24e80` + stubs in OverlayStateDB), Phase 4 real `execute_delegate`/`execute_undelegate` handlers (`a882f65` + `a2ff1cb`), Phase 5 `slash_delegations_for_validator` (`dd21e5c`), Phase 6 `delegated_stake` cache + `refresh_delegated_stakes` (`1805c2e`), Phase 8 13 inline tests (`8c5488d` + `24186e1`). **Open:** Phase 7 `ClaimDelegationTx` to release unbonded amount back to balance — deferred because the new Transaction variant would race against active sister-session edits across `types/lib.rs`, `execution/{lib,parallel,block_stm}.rs`, `consensus/mempool.rs`, `node/{api,persistence}.rs`, and `wallet/{gas,signer}.rs`. Wire it after sister-session merges or stashes its changes. **Wiring deferrals (single-line follow-ups when sister-session calms):** call `ValidatorSet::refresh_delegated_stakes(db)` at start of every block production cycle in `node/main.rs`; call `slash_delegations_for_validator(db, id, 0.10)` next to each `slash_equivocation` site in `tendermint.rs:1113/1377/1473`. | Operators can delegate, undelegate, and get slashed proportionally TODAY. Unbonded amounts cannot be claimed back to balance until Phase 7 lands. Cached `delegated_stake` is 0 until refresh-call wiring lands, so quorum still uses self-stake only on a live cluster |
