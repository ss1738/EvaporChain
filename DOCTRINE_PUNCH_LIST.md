...
| 5 | Lambda-Fold real Nova | ✅ DONE 2026-05-04 + **consumer surface DONE 2026-05-07** + **browser surface DONE 2026-05-08** — full plan in `LAMBDA_FOLD_NOVA_PLAN.md` (36/37 task boxes shipped; only 7.5 arXiv preprint explicitly deferred to academic-press lane per doctrine §A3.3). Real Nova IVC at arity 8 with Poseidon-bound state root + 5-equation chain-aggregate energy-fold gadget. `vk` cached on prover (Phase 3.2); light clients verify via `verify_with_vk_bytes` in **23 ms at 100 folds (1.083× of 10 folds)** on M4 — sublinear claim empirically locked. Soundness tests: `test_real_block_state_root_collision_resistance` (192-bit binding), `test_real_block_energy_fold_rejects_over_reported_decay` (decay over-reporting). End-to-end through tendermint hot path: `test_lambda_fold_nova_end_to_end_three_blocks` at 5.24 s for 3 blocks under release. Governance flag `lambda_fold_mode ∈ {hash_chain, nova}` (default `hash_chain`). HTTP endpoints `/api/lambda_fold/nova{,/verify,/vk_bytes}` shipped on `evaporchain-node`. **Consumer surface 2026-05-07**: `evaporchain-light-client` SDK + `evaporchain-light-client-http` transport + `evaporchain-light-client-cli` operator binary (3 subcommands; cluster-validated 15190→15271 walk on the 5-node WAN soak) + `evaporchain-light-client-example-balance-monitor` worked example + operator runbook (`docs/runbooks/light-client-cli.md`) + READMEs trilogy. Chain endpoints `/api/light_header/{:height,latest}` + `/api/state/proof/:key_hex` shipped (commits `f1b1491`, `e56359a`); `StateDB::prove_at_key` extension shipped across all three backends (`InMemoryStateDB`, `RocksDBStateDB`, `OverlayStateDB`). **Browser surface 2026-05-08**: `evaporchain-light-client-wasm` builds against `wasm32-unknown-unknown` producing a **310 KB .wasm + 26 KB ES-module + TS declarations npm package** (post-`wasm-pack build --target web --release`). Verification runs entirely in-browser with no native C deps. Two architectural refactors enabled this: **Refactor A** — extracted `evaporchain-consensus-types` (Phases 1, 3a, 3b, 2+4+5 across commits `46bfdd4` `3c44eeb` `28a3fba` `f4efdea`) so the SDK no longer transitively pulls `evaporchain-state` → RocksDB; **Refactor B** — feature-flagged BLS backend (`bls-native` blst / `bls-portable` bls12_381 pure Rust) in `evaporchain-crypto` (commit `99bab9c`) so the wasm path uses a wasm32-friendly verifier. **10 cross-backend interop tests** (commit `a5697c6`) prove the portable `bls12_381` verifier produces bit-identical results to blst on real signatures across single-sig + DST handling (PoP / rotation) + 3-signer aggregate-verify (the BFT consensus hot path). The "decade-defining if the math holds" claim from `INVENTION_STACK.md §4.1 row 8` is now operational chain-side AND native-consumer-side AND browser-side — a real signature that verifies on Mini 1 will verify in a browser; a forged signature rejected on Mini 1 will be rejected in a browser. |
| 6 | Ecosystem completion | ✅ **DONE 2026-05-07** (refreshed from "⚠ Partial" — verify-and-tick on 2026-05-07 against the plan-doc bodies confirmed every "deferred" / "in flight" / "remaining" claim below was already shipped via downstream phases; status row was 2-3 days behind code). **Singh-Lyapunov fee controller** ✅ wired. **Crooks-MEV refund** ✅ consensus-integrated 2026-05-04 — full plan in `CROOKS_MEV_INTEGRATION_PLAN.md` (35/35 task boxes SHIPPED including the 4 pre-implementation sanity checks). New `evaporchain-mev-detect` crate; per-block sandwich detector wired into `tendermint.rs::on_block_committed` with Phase 2 rate-based pmf + Phase 3 deterministic digest + Phase 3.3 producer helper + Phase 3.4 validator-rejection rule + Phase 3.5 attacker-debit/victim-credit executor + **Phase 3.5d (validator stake deduction) SHIPPED** via `apply_mev_missing_refund_slashes` gated by `crooks_mev_missing_refund_slash_enabled` (default `false`) + Phase 4 anti-gaming (confidence threshold, self-MEV pre-filter, operator dispute) + **Phase 4.2 (wire-format opt-out) SHIPPED** via `TransferTx::mev_refund_eligible: Option<bool>` with bit-compat serde defaults. Governance flag `crooks_mev_settlement_mode ∈ {observe, enforce}` (default `observe` — current chain bit-compat). HTTP endpoints `/api/mev/observations`, `/api/mev/dispute`. **Light-Cone full DAG** ✅ DONE 2026-05-04 — full plan in `LIGHT_CONE_FULL_DAG_PLAN.md` (39/39 task boxes shipped). DAG-aware tip selection (`MccForkChoice::select_tip` + `TendermintConsensus::current_tip` + `create_proposal` integration), multi-parent block wire-format with hash continuity (`Block::parents` + `Block::effective_parents` + `Block::validate_parents_wire_format`), per-fork state-branch substrate (`state_branches: HashMap<BlockId, LightConeBranchMetadata>` + `LightConeBranchSnapshot` trait + LRU eviction at `light_cone_max_concurrent_forks` cap), Phase 4 substrate (`dag_round_states`, `cross_fork_equivocations`, `committed_at_block`, `is_antichain` + `closing_antichain` primitives), Phase 5 compaction (`prune_orphan_branch` cascade + `detect_orphan_branches` rule + LRU/DAG paired wiring), **Phase 4.1 voting-handler wiring SHIPPED** at `tendermint.rs:4130` (handle_prevote) + `tendermint.rs:4255` (handle_precommit) — both mirror incoming votes into `dag_round_states[tip]` when `light_cone_state_branches_enabled = true` AND the voted-for block is a current DAG leaf, **Phase 4.2 `try_finalize_antichain` predicate SHIPPED** (walks `closing_antichain`, returns leaves with ≥ 2f+1 precommits), end-to-end pipeline test `test_dag_mode_full_pipeline_end_to_end` exercising the 4-fork DAG → LRU eviction → vote-record API → cross-fork equivocation → `try_finalize_antichain` → closing antichain. Governance flags `light_cone_state_branches_enabled` (default `false`), `light_cone_max_concurrent_forks` (1..=8, default 4), `light_cone_orphan_caliber_threshold` (any u64). Decision docs: `research/light_cone/PHASE_3_DECISIONS.md`, `PHASE_4_DECISIONS.md`. |
| 7 | LLSA full / descope | ✅ **Descope path 100% DONE 2026-05-07** (5/5 sub-bullets) — `evaporchain-llsa::apply_amendment` gated chain-side via HTTP endpoint at `api.rs:4694` + integrated into `evaporchain-execution::genesis_invariant`. EPV registry binding works. `MultiAuditorVerifier { verifiers, threshold }` shipped 2026-05-05 (k-of-n auditor signature aggregation, 6 unit tests, replaces `AlwaysAcceptVerifier` stub). M2 Coq build closed 2026-05-05 — Rocq 9.1.1 exit-0 on all 5 `.v` files. CI `make` runs on every PR via the `coq` job in `.github/workflows/ci.yml` against the pinned Rocq 9.1.1 toolchain. Doctrine pitch shipped — `INVENTION_STACK.md §A1.2 T4` reads *"the first chain whose governance is a build-verifiable theorem under audit."* **Bonus 2026-05-07**: the sibling Decay-BFT mechanization in the same Coq corpus also reached zero-Admitted (full BIG theorem `decay_bft_safety_liveness` Qed via 119164b → 86b30c5); the Layer 7 CI gate now also defends the entire 13-obligation Decay-BFT skeleton. Full-path (`CoqVerifier` + on-chain MetaCoq kernel) remains 9-15-month post-V1 work. |
...
| **Tokenomics build-out** | Mainnet-readiness for the chain's economic surface | ⚠ Partial — **3/6 §2 items closed 2026-05-07** (mechanism level). New `TOKENOMICS.md` doctrine doc (549 lines, 28 ceremony questions). **§2.6 Vesting** ✅ wired (`VestingLock` primitive + 7 outflow gates + RocksDB legacy-fallback migration; 9 tests; commit `b666fe7`) + applied to `genesis-mainnet.json` with placeholder schedules locking 900M of 1B EVP at genesis (`bcbb9b0`). **§2.4 Emission schedule** ✅ wired (`Tokenomics.emission: Option<EmissionParams>` dispatcher; `Constant`/`Halving`/`LinearDecay` shapes + max_supply cap; backwards-compat: existing genesis files with no `emission` field route to legacy path → running cluster sees no behavior change; commit `fd1b580`). **§2.3 MEV refund** ✅ reconciled — earlier audit "DOC MENTIONS, NOT WIRED" was a false negative; 11/12 claims verified (`evaporchain-mev-detect` 1,392 LOC + 9 tests, detector wired into `tendermint.rs::on_block_committed`, `due_refund_txs` producer helper, `validate_block_refunds` proposal hook, `execute_refund` attacker-debit/victim-credit, `apply_mev_missing_refund_slashes`, HTTP endpoints); fully wired but operationally inert by default (`crooks_mev_settlement_mode = "observe"` → detection runs, settlement dormant); commit `5136c0a`. **Remaining §2 items ceremony-blocked**: §2.1 (Q6 recipient policy — proposer-only? attesters? delegators?), §2.2 (Q7 validator commission — no commission parameter exists), §2.5 (Q21 staking-APY controller — `target_staking_apy: 0.05` is a dead field). | `9827ce1` (TOKENOMICS.md), `b666fe7` (VestingLock), `bcbb9b0` (genesis vesting), `fd1b580` (emission dispatch), `5136c0a` (MEV reconciliation), `d4f204e` (CHANGELOG arc) — see `TOKENOMICS.md` §1-§3 |
...
- [x] **Unify decay through `evaporchain_types::energy_at_epoch`.** ✅ DONE (commits `4d59b5d` for first 3 rogue implementations; closed end-to-end 2026-05-06 after AUDIT_2026_05_06.md CRITICAL-3 surfaced a 4th workspace-wide violation). All four implementations now route through the canonical Coq-verified function:
  - `crates/evaporchain-consensus/src/anchor.rs:77-91` — `DecayFormula::Exponential::compute_energy` (rerouted in `4d59b5d`)
  - `crates/evaporchain-da/src/poha.rs:99` — `self.energy >> shifts` (rerouted in `4d59b5d`)
  - `crates/evaporchain-self-annealing/src/annealing.rs:54` — `lambda_half_life` shift (rerouted in `4d59b5d`)
  - `crates/evaporchain-half-life-nft/src/token.rs` — local `decay_energy` helper with raw `>> full_halvings` plus linear residual (rerouted 2026-05-06; canonical math was already identical, helper was just the last workspace bypass to eliminate). 5 inline tests updated to call `energy_at_epoch` directly. half-life-nft 28/28 green.
  Workspace lint that fails CI on `>> _` against energy values outside `evaporchain-types` remains open work (still tracked here; the four named violations are now all rerouted, so the lint would currently pass everywhere — it's an enforcement against future regression).
...
- [x] **MCC: decide between (a) re-label Boltzmann as canonical or (b) build real Perron.** ✅ RESOLVED 2026-05-07 evening as **path (a)**. Decision forced by the underlying mathematics, not preference: EvaporChain's `evaporchain_light_cone::LightCone` is a strict DAG, so its adjacency matrix `M` is **nilpotent** — every eigenvalue is 0 — and no nontrivial Perron-Frobenius eigenvector exists for `(I − M)^{-1}`. Perron-Frobenius requires a strongly-connected, irreducible Markov chain; DAG fork-choice doesn't satisfy that hypothesis. Path (b) would produce 200-400 LOC implementing a problem the chain doesn't have. Path (a) — Boltzmann argmax — is the correct closed form for MaxCal under a single thermodynamic constraint (Jaynes 1980; Pressé-Ghosh-Lee-Dill *Rev. Mod. Phys.* 2013), and is what the current `crates/evaporchain-mcc/src/{caliber.rs,choose.rs}` already implements. The "Perron solution" language in `research/INVENTION_STACK.md` §A1.2 T1 should be read as a colloquial Lagrangian-closed-form reference, not a literal eigenvector claim. Source-level rationale at `crates/evaporchain-mcc/src/lib.rs:14-50` is the authoritative documentation. No further code change required — the choice gate closes by ratifying the existing implementation as canonical.
...
- [x] **Light-Cone full consensus — substrate complete; full rewrite remains post-V1.** ✅ Substrate DONE 2026-05-04 (`LIGHT_CONE_FULL_DAG_PLAN.md` Phases 1–6, voting-handler wiring + per-tip `dag_round_states` + `try_finalize_antichain` shipped). Full Tendermint replacement (rewrite of 8,782-LOC `tendermint.rs` behind `trait ConsensusEngine`) remains post-mainnet-V1 work. Substrate sub-items shipped:
  ...
  - ✅ Decay-Lamport clock crate **integrated with LightCone DAG** 2026-05-06. New `evaporchain-light-cone::decay_lamport` module exposes `block_lamport_clock(lc, block_id, tick_quantum) -> LamportClock` and `all_block_clocks(lc, tick_quantum) -> BTreeMap<BlockId, LamportClock>`. Pure function of DAG topology + per-block energies + chain-set tick quantum. Iterative DFS-with-memo (no recursion, 1000-block linear chain test locks stack-safety). Validator-determinism locked by proptest (64 random DAGs). Light-cone suite: 66/66. Closes the previously-deferred *"Decay-Lamport clock crate (separate crate, weeks 20-24)"* item from `block.rs` doctrine comment.
...
- [x] **Parametrize `LLSAInvariantPreservation.v` over `step_new`.** ✅ Closed 2026-05-09 (commit `7d92dd1`). §6b adds `StepMonotone : (ChainState → ChainState → InvParams → Prop) → Prop` (single proof obligation), `step_new_preserves_inv` (generic preservation), `llsa_amendment_gate` (polymorphic gate). `RedirectStep` / `DecayStep` recovered as corollaries via lifted predicates. `coqchk` confirms `Axioms: <none>` under Rocq 9.1.1. Future amendments need only supply (`step_new`, proof of `StepMonotone step_new`, witness) — no edit to the proof file required.
...
**Alt descope path — "audited self-amendment" — 100% DONE 2026-05-07 (5/5):**
...
- [x] Provide pinned Coq toolchain + fix `LLSAInvariantPreservation.v`. ✅ DONE 2026-05-05 (M2). Rocq 9.1.1 build clean for all 5 `.v` files. ✅ CI integration on every PR shipped 2026-05-05 — `coq` job in `.github/workflows/ci.yml` runs `make` on the Mini self-hosted runner against the pinned Rocq 9.1.1 toolchain (path `/opt/homebrew/bin/rocq`); exit-0 required for merge to main. 5-min timeout (real wall-clock build is sub-second on Mini).
...
**Bonus closure 2026-05-07** — the sibling Decay-BFT mechanization in the same Coq corpus reached zero-Admitted on 2026-05-07 (commits 119164b → 86b30c5). The headline theorem `decay_bft_safety_liveness` is now Qed; `EvaporChainSafetyLiveness.v` carries 13 discharged obligations (SAFETY-1/2/3, LIVENESS-1/2, DECAY-1/2, DAG-1/2, HSP, PSP, SAFETY-BASE, LIVENESS-BASE, BIG). The Layer 7 CI gate now defends both Coq tracks — LLSA invariant preservation AND the full Decay-BFT theorem — on every PR.

**Recommendation:** descope to alt path for V1 was the correct call — it shipped, with both Coq tracks now zero-Admitted. Park full LLSA (MetaCoq + extraction-to-Rust on-chain kernel) on the post-mainnet roadmap. Doctrine §A1.2 T4 already reflects the descope-path claim accurately.
