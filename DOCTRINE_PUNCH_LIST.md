...
| **Tokenomics build-out** | Mainnet-readiness for the chain's economic surface | ✅ **6/6 §2 items wired 2026-05-08** (`a6bc9df`). **§2.6 Vesting** ✅ wired. **§2.4 Emission schedule** ✅ wired. **§2.3 MEV refund** ✅ wired. **§2.1 Recipient policy** ✅ wired — `process_block_rewards_v2`: 60% to proposer / 40% split equally among attesters when attesters present; fallback to 100% proposer when none. **§2.2 Validator commission** ✅ wired — `Tokenomics.validator_commission_default: f64` (default 10%, serde-defaulted for genesis compat); `split_staker_pool` splits staker pool into delegator share + validator commission; commission credited to producer (dead-producer tombstone redirect applies). **§2.5 APY cap controller** ✅ wired — `Tokenomics.apy_capped_reward` scales block reward down when projected annual yield exceeds `target_staking_apy × total_staked`; both `process_block_rewards_v2` call sites in lib.rs + parallel.rs pass real `total_staked`. All engineering §2 items closed; ceremony questions (Q1-Q28 in `TOKENOMICS.md`) remain for tokenomics advisors. | `9827ce1` (TOKENOMICS.md), `b666fe7` (§2.6 Vesting), `fd1b580` (§2.4 Emission), `5136c0a` (§2.3 MEV), `a6bc9df` (§2.1+§2.2+§2.5 bundle) |
...
- [x] **Promote conservation audit from gating to mandatory** DONE -- default changed to "enforce" in commit 76d95590 (T1.13, 2026-05-15). Clusters start in enforcement mode. (sequel to Layer 0 first item — once Layer 4 changes block acceptance semantics, revisit the governance flag).
