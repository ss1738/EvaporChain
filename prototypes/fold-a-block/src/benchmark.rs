...
impl BenchmarkReport {
    ...
    pub fn print_report(&self) {
        println!("\n{}", "=".repeat(60));
        println!("  EVAPORCHAIN FOLD-A-BLOCK BENCHMARK RESULTS");
        println!("{}\n", "=".repeat(60));
        ...
        println!("Folding ({} blocks):", self.blocks_folded);
        println!("  Average:        {:.3}ms", self.avg_fold_ms);
        println!("  Median:         {:.3}ms", self.median_fold_ms);
        println!("  Min:            {:.3}ms", self.min_fold_ms);
        println!("  Max:            {:.3}ms", self.max_fold_ms);
        println!("  P95:            {:.3}ms", self.p95_fold_ms);
        println!("  P99:            {:.3}ms", self.p99_fold_ms);
        ...
        let avg_ms = self.avg_fold_ms;
        match self.verdict.as_str() {
            "PASS" => {
                println!("VERDICT: PASS -- Fold time {:.1}ms < 10ms. EvaporChain is feasible.", avg_ms);
            }
            "MARGINAL" => {
                println!(
                    "VERDICT: MARGINAL -- Fold time {:.1}ms. Feasible with batching.",
                    avg_ms
                );
            }
            _ => {
                println!(
                    "VERDICT: FAIL -- Fold time {:.1}ms exceeds 50ms threshold.",
                    avg_ms
                );
            }
        }
