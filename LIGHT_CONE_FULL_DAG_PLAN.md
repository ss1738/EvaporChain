...
- [x] **3.2 — Per-tip executor (consensus-side seam + executor-side wiring).** SHIPPED end-to-end. `LightConeBranchSnapshot` trait (`tip()`, `created_at_height()`) — minimal abstraction the executor implements; consensus engine doesn't need to depend on `evaporchain-state` to wire it. `LightConeBranchMetadata.snapshot: Option<Arc<dyn LightConeBranchSnapshot + Send + Sync>>` field carries the ref. `TendermintConsensus::attach_branch_snapshot(tip, snapshot)` installs it (returns `None` if the tip isn't tracked — caller must `record_state_branch` first). **Executor-side wiring CLOSED 2026-05-06**: `TendermintConsensus::capture_committed_branch_snapshot(block, db)` calls `StateSnapshotBranch::capture` (full-state copy via `SnapshotBuilder::create`) and threads the resulting `Arc<dyn LightConeBranchSnapshot>` into `attach_branch_snapshot`. Wired into `apply_block` (the production sync-path commit hook) immediately after `on_block_committed` — `record_state_branch` populates the metadata, then the capture method materializes the snapshot. No-op when `light_cone_state_branches_enabled != "true"`; defensive no-op when the tip isn't yet tracked (flag-flip race). 3 regression tests: `test_capture_committed_branch_snapshot_attaches_to_recorded_tip`, `…_noop_when_flag_off`, `…_noop_when_tip_not_tracked`. Consensus suite: 514 / 0 / 1.
