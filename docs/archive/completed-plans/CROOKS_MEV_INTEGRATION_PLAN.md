...
- [x] **7.4 — Operator runbook**: SHIPPED. Standalone `docs/runbooks/crooks-mev-enable.md` shipped 2026-05-07 — covers the three settlement stages (Stage 0 default observe, Stage 1 enforce-mode flip, Stage 2 slashing enable), per-stage pre-flight checklists, governance-flag procedures for both `crooks_mev_settlement_mode` and `crooks_mev_missing_refund_slash_enabled`, operator-endpoint reference (`/api/mev/observations`, `/api/mev/dispute`, `/api/governance/flags`), and rollback procedures. Cross-references `CROOKS_MEV_INTEGRATION_PLAN.md`, `INVENTION_STACK.md §A1.3`, the three implementation crates, and the related disaster-recovery runbook.

**Phase 7 deliverable: SHIPPED (4/4).** Layer 6 line item flipped, INVENTION_STACK row updated, whitepaper §8 reframed to two-tier defense, operator runbook shipped at `docs/runbooks/crooks-mev-enable.md` (2026-05-07).
