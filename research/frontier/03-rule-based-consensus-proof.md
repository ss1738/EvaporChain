...
If any of `QueryDeterminism`, `MonotoneDecay`, `BoundedByInitial`, or `AnchorSanity` fails under TLC, the integer `LazyEnergy` formula in `crates/evaporchain-types/src/lib.rs` (canonical `energy_at_epoch`; pre-fix this doc cited `evaporchain-state/src/evaporation.rs` which does not define the function) has a bug. That would be the kind of bug an audit firm should find.
...
1. Read the Rust implementation `crates/evaporchain-types/src/lib.rs` (canonical `energy_at_epoch`; pre-fix this doc cited `evaporchain-state/src/evaporation.rs` which does not define the function) and confirm `energy_at_epoch` matches the formula in `LazyEnergy` (§3 of the .tla file).
...
- `crates/evaporchain-types/src/lib.rs` (canonical `energy_at_epoch`; pre-fix this doc cited `evaporchain-state/src/evaporation.rs` which does not define the function) — Rust implementation of `LazyEnergy`
