...
- **Quorum at Active.** `is_supermajority()` at `da/poha.rs:149-151` enforces `attested_stake * 3 > total_stake * 2` (strict after Q4 audit closure). The spec encodes this as a parameterised quorum (default 2/3) that all Active certs must satisfy.
...
Combined with the production code's BLS signature verification at `da/certificate.rs` and the supermajority check at `da/poha.rs:149`, this gives a **lifecycle-correct, signature-verified** PoHA implementation — assuming attesters are honest about their sampling claims.
