...
|---|---|---|---|
| 1 | **Light-Cone Consensus** ✅ SUBSTRATE-COMPLETE 2026-05-04 (`LIGHT_CONE_FULL_DAG_PLAN.md` Phases 1-3+5 shipped, Phase 4 substrate shipped) | Causal-set partial-order consensus (Sorkin/Pratt). Energy decay gives the time arrow. **Substrate scope (audit 2026-05-17):** DAG-aware tip selection + multi-parent wire format + per-fork state branches + LRU/orphan-prune lifecycle + antichain primitives all landed *as observability and substrate*; voting-handler wiring (per-tip `dag_round_states` dispatch + antichain finalization) is the remaining consensus-state-machine surgery — until that lands, the partial-order is recorded and inspectable but is **not** the chain's fork-choice rule. Promotion to fork-choice is Layer 4 work (per `DOCTRINE_PUNCH_LIST.md`). Governance flag `light_cone_state_branches_enabled` (default off — chain bit-compat). | Consensus |
