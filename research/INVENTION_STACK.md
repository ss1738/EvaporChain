...
| **T3** | **Causal-State Light Client (CSLC)** — ε-machine reconstruction of the energy-filtered tx process | Shalizi-Crutchfield 2001 (Optimal Prediction Theorem); Shalizi-Klinkner 2004 (CSSR). Algorithm shipped at `evaporchain-cslc::cssr::reconstruct_cssr` (705 LOC: sliding-window history counts + χ² independence test with hardcoded critical values for α ∈ {0.001, 0.005, 0.01, 0.05} + 3-phase homogenize/determinize loop). 4/5 punch-list acceptance criteria pass: fair coin → 1 state, period-2 → 2 states, golden-mean → 2 states with both pmfs within ε=0.02 TV-distance of reference at α=0.001, 50k symbols. Even-process Phase II precision (over-splits ~2× canonical) is the open Layer 2 follow-up. | "Our light clients carry the *unique minimal sufficient predictive model* of the energy-surviving tx process — Shalizi-Crutchfield's Optimal Prediction Theorem proves the ε-machine is provably optimal (any predictor with fewer states cannot match it). EvaporChain ships the full Shalizi-Klinkner CSSR reconstruction algorithm at α=0.001 with empirical recovery of the canonical fair-coin / period-2 / golden-mean machines on 50k-symbol streams, including pmf TV-distance verification. The even-process determinization-precision case is the last open work; the headline claim — *theorem-grade light-client predictor* — is now load-bearing on real code, not a stub." |
