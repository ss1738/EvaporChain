...
|---|---|---|---|
| 1 | **Light-Cone Consensus** ✅ SUBSTRATE-COMPLETE 2026-05-04 (`LIGHT_CONE_FULL_DAG_PLAN.md` Phases 1-3+5 shipped, Phase 4 substrate shipped) | Causal-set partial-order consensus (Sorkin/Pratt). Energy decay gives the time arrow. *Soul of the chain.* DAG-aware tip selection + multi-parent wire format + per-fork state branches + LRU/orphan-prune lifecycle + antichain primitives all landed; voting-handler wiring (per-tip `dag_round_states` dispatch + antichain finalization) is the remaining consensus-state-machine surgery. Governance flag `light_cone_state_branches_enabled` (default off — chain bit-compat). | Consensus |
