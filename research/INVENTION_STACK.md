...
|---|---|---|---|
| 1 | **Light-Cone Consensus** ✅ SUBSTRATE-COMPLETE 2026-05-04 (`LIGHT_CONE_FULL_DAG_PLAN.md` Phases 1-3+5 shipped, Phase 4 substrate shipped) | Causal-set partial-order consensus (Sorkin/Pratt). Energy decay gives the time arrow. *Soul of the chain* (intent — see production status caveat below). DAG-aware tip selection + multi-parent wire format + per-fork state branches + LRU/orphan-prune lifecycle + antichain primitives all landed; voting-handler wiring (per-tip `dag_round_states` dispatch + antichain finalization) is the remaining consensus-state-machine surgery. Governance flag `light_cone_state_branches_enabled` (default off — chain bit-compat). **Production status:** read-only observability layer as of 2026-05-03 — Tendermint is the chain authority until voting-handler wiring lands (`DOCTRINE_PUNCH_LIST.md` Layer 4). No consensus-rejecting decision currently consults the DAG/antichain machinery. See `crates/evaporchain-light-cone/src/lib.rs` for the authoritative production-status note. (INV-MED-4 audit 2026-05-17) | Consensus |
...
|---|---|---|---|
| **T1** | **Maximum-Caliber Consensus (MCC)** — fork-choice rule selecting the chain trajectory that maximizes path-entropy under ⟨ΔE⟩ = λ. Single-line trajectory walk shipped at `evaporchain-consensus::fork_choice::MccForkChoice` behind `parent_acceptance_mode = "mcc"` (Lane I.3 + I.4 + I.6); full multi-parent enumeration substrate (Phases A + B of `MCC_FULL_MULTI_PARENT_PLAN.md`) shipped 2026-05-05 with 34 tests covering candidate-head enumeration, planning, snapshot capture/restore, and atomic transactional replay. Hot-path integration (Phase C) + adversarial testing (Phase D) remain. | Jaynes 1980 (Maximum Caliber); Pressé-Ghosh-Lee-Dill *Rev. Mod. Phys.* 2013 | "Our fork choice is the unique trajectory `argmax exp(−β·E_path)` over candidate chain trajectories — closed form by Lagrange duality on the maximum-entropy program. (Note: a Perron-Frobenius solution would require a strongly connected graph; the LightCone DAG is acyclic, so adjacency is nilpotent and Perron is vacuous. The Lagrangian `argmax` is what's actually shipped.) Multi-parent enumeration substrate shipped: `enumerate_candidate_heads` returns every active sibling head with its caliber score, `plan_replay_to_head` produces the canonical replay walk between any two heads, `replay_and_apply_atomic` provides transactional state-replay across branch switches with full rollback semantics. Once Phase C wires this into the consensus hot path (start_round → vote dispatch → proposer parent-set selection), MCC becomes the *load-bearing* fork-choice across multi-parent DAGs, not just the single-line trajectory walker shipped today." |
| **T2** | **Crooks-Singh Fee Equilibrium (CFM)** — closed-form fee distribution `p_eq(f) ∝ exp(−β f) · ρ_mempool(f)` with `β = 1/λ` | Crooks 1999 (Fluctuation Theorem); Jarzynski 1997 | "Our fee market exposes the Crooks identity primitive `log(p_F / p_R) = β·(W − ΔF)` — implemented as `crooks_log_ratio_millibits(p_F, p_R)`. The chain ships the LHS AND the soft-variant RHS-equality test (synthetic forward/reverse trajectory pair, assert equality to fixed-point precision in `crates/evaporchain-cfm/src/crooks.rs::identity_holds_for_synthetic_forward_reverse_pair`, closed in commit `d80921f` per DOCTRINE_PUNCH_LIST.md Layer 2). The *hard variant* — building a stochastic-thermodynamics driver that produces actual Crooks-distributed forward/reverse work distributions across real chain trajectories — remains an open multi-week research task; the chain's current claim is honestly scoped to *exposed primitive + synthetic-pair identity verification*." |
| **T3** | **Causal-State Light Client (CSLC)** — ε-machine reconstruction of the energy-filtered tx process | Shalizi-Crutchfield 2001 (Optimal Prediction Theorem); Shalizi-Klinkner 2004 (CSSR). Algorithm shipped at `evaporchain-cslc::cssr::reconstruct_cssr` (705 LOC: sliding-window history counts + χ² independence test with hardcoded critical values for α ∈ {0.001, 0.005, 0.01, 0.05} + 3-phase homogenize/determinize/merge). 4/5 punch-list acceptance criteria pass: fair coin → 1 state, period-2 → 2 states, golden-mean → 2 states with both pmfs within ε=0.02 TV-distance of reference at α=0.001, 50k symbols. The even-process over-splits ~2× canonical (4 states vs 2); **root cause diagnosed 2026-05-05** (`cssr_even_process_state_pmf_dump`): the algorithm recovers pmfs `[67/33, 75/25, 50/50, 100/0]` where states 50/50 + 100/0 ARE canonical E + O, but 67/33 (empty-history marginal at steady-state π=(2/3,1/3)) and 75/25 (`P(X_t | X_{t-1}=0)` posterior mixture) are statistical-mixture artifacts. The χ² test correctly distinguishes them from the pure states — Phase III merge therefore correctly does NOT collapse them. Proper fix is **multi-week research-grade algorithmic redesign**: convex-combination mixture detection, OR Bayesian credible intervals (Strelioff-Crutchfield 2014), OR strict L-grow-on-split semantics. Open as a research thread, not a punch-list item. | "Our light clients carry the *unique minimal sufficient predictive model* of the energy-surviving tx process — Shalizi-Crutchfield's Optimal Prediction Theorem proves the ε-machine is provably optimal (any predictor with fewer states cannot match it). EvaporChain ships the full Shalizi-Klinkner CSSR reconstruction algorithm at α=0.001 with empirical recovery of the canonical fair-coin / period-2 / golden-mean machines on 50k-symbol streams, including pmf TV-distance verification. The even-process specifically requires mixture-aware reconstruction (open research); the headline claim — *theorem-grade light-client predictor* — is load-bearing on real code for the cases where the predictive equivalence classes are not statistical mixtures." |
| **T4** | **Lambda-Locked Self-Amendment (LLSA)** — protocol upgrades require a Coq/Lean term of type `forall s, Inv(s) → Inv(step_new(s))` | Coq 9.1.1 build-verifiable kernel proof (`research/proofs/LLSAInvariantPreservation.v`) + k-of-n auditor attestation via `MultiAuditorVerifier`. Full theorem-grade path (pinned MetaCoq + extraction-to-Rust on-chain kernel) remains 9-15-month post-V1 work. | "Our upgrades are gated by audited mechanically-checked invariant-preservation proofs — every amendment carries a Coq term that any auditor named in genesis can re-run locally; governance accepts iff k-of-n auditors sign attestations of `term-typechecks ∧ matches-on-chain-hash`. Tezos has self-amendment without proofs; EvaporChain ships the *audited* form today (descope path: Coq build + `MultiAuditorVerifier` k-of-n threshold), with the *fully theorem-grade* on-chain kernel path tracked as post-V1 work. We are the first chain whose governance is a build-verifiable theorem under audit." |
...
| **Crooks-MEV Refund** ✅ CONSENSUS-INTEGRATED 2026-05-04 (`CROOKS_MEV_INTEGRATION_PLAN.md` Phases 1–5) | Crooks 1999 fluctuation-theorem ratio | refund formula falls out of CFM; fair restitution = ΔF computed from forward/reverse work distributions. Per-block sandwich detector + rate-based pmf + deterministic digest + producer helper + validator-rejection rule + executor attacker→victim balance movement + anti-gaming (confidence threshold, self-MEV pre-filter, operator dispute via `POST /api/mev/dispute`). Governance flag `crooks_mev_settlement_mode ∈ {observe, enforce}` (default `observe` — chain bit-compat). New crate `evaporchain-mev-detect` + executor `execute_refund` + HTTP endpoints `/api/mev/{observations, dispute}`. Phase 3.5d (validator stake deduction) + Phase 4.2 (wire-format `mev_refund_eligible` opt-out) + Phase 6 (e2e perf test) + Phase 4.4d (cluster-wide dispute consensus) deferred to dedicated sessions. |
