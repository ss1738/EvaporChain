...
| D7 | Key rotation / jailing / tombstoning / epoch transitions unmodeled in TLA | MED → **HIGH (finding)** | **PR #458 + #462** | Manager verified (PR #458); cross-epoch safety modeled (PR #462) — **FOUND count-churn gap** | DONE (finding filed) |
...
- **D7 fully addressed (PR #458 + #462):** EpochTransitionManager verified (PR #458, 59K states); cross-epoch agreement modeled (PR #462, `CrossEpochAgreement.tla`) — which **found a real safety gap** (see below).
- **0 drifts OPEN.** Every original D1-D14 finding plus the D3-MCC and D7-Part2 follow-ups is closed/shipped.
- **2 NEW impl findings surfaced by the new specs:**
  1. **MCC fork-choice (PR #460 → FIXED in PR #461):** `mcc_choose` (larger-id) vs `select_tip` (smaller-id) tie-break mismatch + a β-source mismatch (proposer β=1000 vs acceptance β≈244). Both could reject a correctly-proposed tip once MCC mode is enabled. **Fixed** (owner-confirmed: β = `1e6/half_life` everywhere via a shared helper; smaller-id tie-break) and verified (consensus lib 986 pass / 0 fail). Task #37.
  2. **Cross-epoch agreement (PR #462, OPEN finding):** EvaporChain's churn cap is COUNT-based but quorums are STAKE-weighted. Count-bounded churn does NOT guarantee stake-quorum intersection across an epoch boundary — TLC found a config (one validator leaves + one stake-update) where an epoch-N quorum and an epoch-N+1 quorum intersect only in a Byzantine validator → cross-boundary equivocation → reorg of finalized history. Equal-stake control run is fully safe (316 states), pinpointing the count-vs-stake mismatch + uncapped stake updates as the cause. **Needs owner fix** (bound stake churn / require >1/3 stake overlap) before enabling dynamic validator sets. Gated; no current live impact. Task #38.

**This session took the drift sweep from 14 open findings to a COMPLETE close-out** across 13 PRs + 1 audit-doc PR, creating 6 new exhaustively-verified spec files (`CrooksMEV`, `CrossForkEquivocation`, `ValidatorSetTransition`, `AntichainConsensus`, `MccForkChoice`, `CrossEpochAgreement`), tightening `EvaporChainBFT.tla` (stake-weighted quorum + 3-gate LockSafety fix), and **fixing one real consensus bug (MCC, PR #461) + surfacing one more (cross-epoch stake churn, PR #462)** that pure spec-vs-impl review would not have caught.
