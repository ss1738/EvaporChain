// OracleFeed — fourth pilot EvaporScript contract.
//
// Standard oracles publish (value, timestamp) and force every consumer
// to decide staleness for themselves. OracleFeed inverts that: the feed
// itself is a decaying contract, and `max_age` is a hard ceiling on
// how old a value is allowed to be before `is_fresh` flips false.
// Stale data can still be read for forensics, but the contract makes
// freshness a structural property of the read, not a consumer
// convention.
//
// Deploy via DeployScript with energy sized for the operator's
// update cadence. The deployer is the operator (`caller == owner`),
// and `set_feed(label, max_age)` arms the feed exactly once.
