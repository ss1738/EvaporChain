...
|---|---|---|---|
| T3.1 | Phase C cluster deploy + bring-up | 🟢 **CAN CLOSE 2026-06-04 — four CRITICAL bugs surfaced + closed in a single session; cluster sustained 250+ blocks under live BFT consensus**. Bring-up arc: (1) #1 halted h=201 → DA-BLS verify regression fixed (`c3ec29ef` + `ceb95025`); (2) #2 forked h=84 → post-cert block-field mutation fixed (`6db4aca1`); (3) #3 stalled h=202 → P2-04 stuck-proposer + state-sync gap=1 + RequestSync upper-bound off-by-one all fixed (`47a379e1` + `5773fc5e` + `dca50704`); (4) #4 sustained past h=250 across all 3 Minis with **zero** cert-vs-actual mismatch, **zero** parent-hash mismatch, **zero** DA verify failures, and a self-healing P2-04+sync recovery cycle (~10 P2-04 fires + 20 successful syncs per 250 blocks — adds ~4% latency but maintains progress). The 24-hour soak acceptance criterion is now actually achievable; no permanent halt point known. See `FINDING_DA_BLS_VERIFY_2026_06_04.md` + `FINDING_P2_04_LIVENESS_LAG_2026_06_04.md`. Reusable launch script: `scripts/launch-colo-3node-cluster.sh`. | OPS |
...
| T0.2 | Layer 4 D-track adversarial + perf + 72hr soak | 🔴 SOAK-BLOCKED — prior "T3.1 ✅; cluster live" is FALSE (verified 2026-05-18, T3.1 REGRESSED, 0/5 nodes). Perf + 72hr soak need a representative multi-box cluster (operator cost decision) — a co-located soak on the 2-vCPU permanent VPS is non-representative and must NOT be claimed as this. Adversarial/functional D-track scenarios can run as integration tests today. | CONSENSUS |
...
| T0.6 | Slashing-at-scale empirical tests | 🟢 **FULLY VALIDATED 2026-06-20** — 9/9 in-process scenarios green (5 prior + 4 BLS-signed: S6, S7, two controls; commits `5401b571` + `39712cbd`). Live-cluster admin-API slash (2026-06-04). **NEW**: **consensus-automatic Byzantine slashing demonstrated end-to-end on the live 3-Mini cluster 2026-06-20** — M3 launched with `EVAPORCHAIN_BYZANTINE_DOUBLE_PRECOMMIT=true` emitted real equivocating precommits at h=1; M1 + M2 detected via gossiped Precommit handler + equivocation gate, both slashed M3 to stake=0 + jailed within **15ms gossip-to-converge latency**, identical slash amounts on both honest nodes (deterministic Sanov-KL). Reports: `docs/runbooks/t0.6-slashing-at-scale-report-2026-06-04.md` (admin path) + `docs/runbooks/t0.6-byzantine-soak-report-2026-06-20.md` (Byzantine path). Byzantine flag commits: `abaeb32c` (field + tick-path inject + node wiring + mainnet refusal) + `dff78340` (per-msg-path mirror). Only remaining T0.6-adjacent item: representative perf 72hr soak — gates T0.2, not T0.6, requires operator scope-decision for ≥1 paid VPS. | EXECUTION + STATE-DB |
...
| T1.18 | Validator-key passphrase migration on live nodes | ✅ **DONE 2026-06-04** — full env-var → file-form migration validated on the 3-Mini colo cluster (commit `a8f786f3`). Keys re-encrypted to EVK1 (T1.19's pattern), passphrase written to `<data_dir>/validator_pass` at mode 0600, cluster launched with `EVAPORCHAIN_VALIDATOR_KEY_PASS_FILE=<path>` and NO `EVAPORCHAIN_VALIDATOR_KEY_PASS` env var, all 3 nodes loaded keys + matched genesis pubkeys + advanced past h=200 with full 3/3 BFT quorum. Plaintext keys restored on the cluster afterwards. Report: `docs/runbooks/validator-passphrase-file-form-report-2026-06-04.md`. | OPS-RUNBOOK |
| T1.19 | EVPL plaintext key migration on live nodes | ✅ **DONE 2026-06-04** — full plaintext→EVK1 migration validated end-to-end on the 3-Mini colo cluster. Per-Mini `encrypt-bls-key`: 32-byte plaintext → 92-byte EVK1; cluster relaunched with `EVAPORCHAIN_VALIDATOR_KEY_PASS` set; all 3 nodes loaded keys + matched genesis pubkeys + advanced 200+ blocks under full 3/3 BFT quorum. `decrypt-bls-key` round-trip recovered bit-identical plaintext (md5 match). Report: `docs/runbooks/evpl-plaintext-migration-report-2026-06-04.md`. Plaintext keys restored on the cluster afterwards so default state is unchanged. | OPS-RUNBOOK |
...
[2026-05-18T00:00Z] Sonnet 4.6 · T3.1 RE-VERIFIED DOWN — 0/5 nodes serving on Minis+Hetzners; permanent single-node anchor http://89.167.52.40:8099 is the live-verify surface for all deploy scripts
[2026-05-18T02:00Z] Sonnet 4.6 · EvaporScript stdlib sessions 5-6 — cap_decay_vm.es, dp_native_vm.es, total_evaporscript_vm.es, ssm_vm.es — all 4 contracts created, 4 deploy scripts live-verified (8 modes, 4 CIDs: 53/55/57/58/60/61) on 89.167.52.40:8099
[2026-05-18T04:00Z] Sonnet 4.6 · EvaporScript stdlib session 7 — oracle_feed.es, multisig.es — 2 contracts, 2 deploy scripts, 4 modes (CIDs 62/63/64/68) live-verified; two EvaporScript gotchas documented (address map key coercion, TX hash dedup on rejected TXs)
[2026-05-18T06:00Z] Sonnet 4.6 · EvaporScript stdlib session 8 — bounty.es, vesting_schedule.es — 2 contracts, 2 deploy scripts, 4 modes (CIDs 70/72/73/74) live-verified; untag Bool=false bug fixed; caller==self.field address comparison confirmed working
[2026-05-18T08:00Z] Sonnet 4.6 · EvaporScript stdlib session 9 — lottery.es, sealed_bid_auction.es — 2 contracts, 2 deploy scripts, 4 modes (CIDs 77/79/80/81) live-verified; TX dedup symmetry fully documented (symmetric applies to accepted AND rejected TXs)
[2026-05-18T10:00Z] Sonnet 4.6 · EvaporScript stdlib MILESTONE — 10 contracts live-deployed, 20 modes live-verified end-to-end on permanent node 89.167.52.40:8099 · stdlib covers: SSM, TotalVM, CapDecayVM, DpNativeVM, OracleFeed, Multisig, Bounty, VestingSchedule, Lottery, SealedBidAuction
[2026-05-18T10:30Z] Sonnet 4.6 · Research papers read: whitepaper.md, paper_1_mechanism.md, paper_2_state_economics.md, IMPOSSIBLE_RESEARCH_STACK.md, INVENTION_STACK.md, all 5 Coq proofs, all 5 TLA+ specs, 3 frontier papers, 4 dApp architecture docs (SFSV/EvaporCash/DeadDrop/GDPR)
```
