...
| T0.2 | Layer 4 D-track adversarial + perf + 72hr soak | 🟡 OPEN — T3.1 ✅; cluster live; D.1-D.5 soak ready to run | CONSENSUS |
...
| T0.6 | Slashing-at-scale empirical tests | 🟡 **OPEN — cluster soak ready** — all 5 adversarial scenarios in `crates/evaporchain-consensus/tests/slashing_at_scale.rs` (prevote-equivocation, precommit-equivocation, MEV-missing-refund, downtime-proportional, multi-validator cascade) — 5/5 green on Mini 1 2026-05-14. T3.1 ✅; live-cluster soak can now run. | EXECUTION + STATE-DB |
