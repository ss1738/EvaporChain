...
|---|---|---|---|
| T3.1 | Phase C cluster deploy + bring-up | 🟡 **PARTIAL 2026-06-04 — three sequential CRITICAL fixes landed, cluster advanced 201 blocks cleanly, new minor liveness issue at h≈202**. (1) Bring-up #1 halted at h=201 → DA-attestation BLS verify regression diagnosed + fix landed (test `c3ec29ef`, fix `ceb95025`). (2) Bring-up #2 advanced to h=83 → fork-on-mismatched-parent-hash at h=84 diagnosed: gossip-receiver path was mutating `block.state_root` post-cert + local-propose path was mutating `block.post_state_root`; both fields are inside `block_hash()`, so the persisted block's hash drifted from the BLS-signed cert.block_hash, and the next block's `self.parent_hash` (derived from state_root) diverged per node. Fix in commit `6db4aca1` drops both mutations. (3) Bring-up #3 ran 201 blocks cleanly with **zero** cert-vs-actual-hash mismatches, **zero** parent-hash mismatches, **zero** DA verify failures across all 3 nodes — both bugs confirmed closed end-to-end. Cluster stalled at h=202 on a separate, less-severe issue (precommit timeouts; M1 fell one block behind; no consensus-determinism warning fires) — likely peer-sync flakiness rather than another non-determinism bug. T0.6 live-soak now unblocked at the bug level; the h=202 follow-up needs short investigation but the lane's core blockers (DA-BLS + post-cert mutation) are closed. Reusable launch script at `scripts/launch-colo-3node-cluster.sh`. See `FINDING_DA_BLS_VERIFY_2026_06_04.md` for the full diagnostic + resolution trail. | OPS |
