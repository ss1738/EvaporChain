...
| T0.6 | Slashing-at-scale empirical tests | 🟢 **EMPIRICAL RE-VALIDATED on current main 2026-05-18** — all 5 scenarios in `crates/evaporchain-consensus/tests/slashing_at_scale.rs` (prevote-equiv, precommit-equiv, MEV-missing-refund, downtime-proportional, multi-validator cascade) **5/5 green on origin/main `ada8258d`, VPS** (supersedes the stale "Mini 1 2026-05-14"). Slashing logic verified on today's code. Remaining = live-cluster soak ONLY, which is 🔴 (prior "T3.1 ✅" FALSE — see T0.2; representative soak needs operator multi-box infra, NOT a co-located 2-vCPU box). | EXECUTION + STATE-DB |
