...
| T0.8 | Light-client / fast-sync against malicious snapshots | 🟡 **PARTIAL** — sub-task 4 partial-state-withhold rejected via quorum cert ✅ (`8abd388`, 2026-05-11); structural validation on `from_bytes` closes duplicate-id gap ✅ (`dee358b`, 2026-05-11). Remaining: full edge-case coverage on the malicious-snapshot envelope. | NETWORK |
| T0.9 | Bridge Phase 4 full V2 (Halo2 EccChip in-circuit Pallas MSM) | 🟡 **PARTIAL** (7/8 sub-tasks) — A/B-starter/B-finish/C-native/C-circuit-starter/C-circuit-FINISH/**D-starter** all ✅. **D-finish** (real IPA prove/verify) blocked on curve-param resolution: `Params<EqAffine>` reports `Circuit<Fq>` is required despite `EqAffine::ScalarExt = Fp`. Hypothesis to test next: use `Params<EpAffine>` (pallas::Affine) instead — Halo2 may bind `C::Scalar` to BASE field, not Scalar. Most recent: dd75ba6 (D-starter wire format + JSON round-trip; 22/22 on-host). | BRIDGE-RUST |
