...
| T0.6 | Slashing-at-scale empirical tests | 🟡 **CODE-COMPLETE — CLUSTER-ONLY** — unit-scale coverage now present for all 5 rules: (1) double-vote `test_byzantine_double_vote_{prevote,precommit}_detected` in `byzantine_adversarial.rs` [parallel-session uncommitted in working tree as of 2026-05-11]; (2) equivocation across rounds `test_slash_equivocation{,_jails_if_below_min}` + production `sanov_slash_equivocation` at `tendermint.rs:2057`; (3) MEV missing-refund `test_apply_mev_missing_refund_slashes_{flag_gated,applies_slash}` + production at `tendermint.rs:2304`; (4) key rotation grace expiry `test_purge_expired_prev_keys_clears_stale_entries` + production `purge_expired_prev_keys` at `validator-set.rs`; (5) Mortis partial trigger `partial_trigger_resets_then_re_fires` at `mortis/src/monitor.rs` (`406806d`, 2026-05-11). Live-cluster adversarial harness (5 scenarios producing slash + log under real Byzantine conditions) still gated on T3.1. | EXECUTION + STATE-DB |
