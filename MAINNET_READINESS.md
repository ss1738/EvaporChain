...
|---|---|---|---|
| T3.1 | Phase C cluster deploy + bring-up | 🟡 **PARTIAL 2026-06-04 — DA-BLS bug FIXED + verified, NEW non-determinism bug surfaced at h≈84**. Bring-up #1 (commit `23f0eafd`) halted at h=201 (DA enforcement boundary) due to BLS DA-attestation verify regression; root cause + fix landed same day (regression test `c3ec29ef`, fix `ceb95025`). Bring-up #2 (commit `ceb95025`) confirmed the DA-BLS fix: zero verify failures across all 3 nodes, chain advanced from h=0 to h=83 with full 3/3 BFT quorum. **NEW issue at h≈84**: proposal-parent-hash mismatch (`local_parent=b168... vs proposal_parent=54ee...`), validators forked on h=83 hash despite identical commit history through h=73. Likely state-root non-determinism or block-payload encoding drift. T0.6 live-soak still gated, now on the new non-determinism bug. Reusable launch script at `scripts/launch-colo-3node-cluster.sh`; cluster currently stopped. See `FINDING_DA_BLS_VERIFY_2026_06_04.md` for both findings. | OPS |
