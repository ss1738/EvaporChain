...
| T0.6 | Slashing-at-scale empirical tests | 🟢 **EMPIRICALLY VALIDATED 2026-06-04** — 7/7 in-process scenarios green on Mini-1 (5 prior + 2 new BLS-signed regressions: `t06_scenario_6_bls_signed_precommit_equivocation_slashes_and_jails` + control test pinning BLS-verify-runs-before-equivocation-gate, commit `5401b571`). Live-cluster admin-API slash demonstrated end-to-end: val-3 stake mutated 250000 → 225000 → 45000 → 0 under three Sanov slashes (downtime 5/100, downtime 20/100, equivocation full) with the chain continuing to advance, no fork, cluster integrity preserved. Report: `docs/runbooks/t0.6-slashing-at-scale-report-2026-06-04.md`. Remaining gaps: (a) consensus-AUTOMATIC slashing under gossiped Byzantine evidence (needs a feature-gated Byzantine binary build, ~2-3 hours, tractable as a follow-up); (b) representative perf 72hr soak (still needs operator scope-decision for ≥1 paid VPS — gates T0.2's perf soak too). Neither is a code-correctness gap. | EXECUTION + STATE-DB |
