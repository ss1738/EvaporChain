        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3">
          ...
          {selectedDetail.metadata && Object.keys(selectedDetail.metadata).length > 0 && (
            <div className="px-3 py-3 rounded-lg bg-evap-surface border border-evap-border">
              <p className="text-[10px] font-semibold text-zinc-400 mb-2">Metadata</p>
          ...
          {selectedDetail.energy_history && selectedDetail.energy_history.length > 0 && (
            <div className="px-3 py-3 rounded-lg bg-evap-surface border border-evap-border">
              <p className="text-[10px] font-semibold text-zinc-400 mb-2">Energy Over Lifetime</p>
          ...
          {selectedDetail.proof_status !== "expired" && (
            <div className="px-3 py-3 rounded-lg bg-evap-green/5 border border-evap-green/20">
              <p className="text-[11px] font-semibold text-zinc-200 mb-2">Resurrect This Object</p>

              <div className="flex items-center gap-2 mb-2">
                <label className="text-[10px] text-zinc-400">Energy deposit:</label>
                <input
                  type="number"
          ...
          {selectedDetail.proof_status === "expired" && (
            <div className="px-3 py-3 rounded-lg bg-red-500/5 border border-red-500/20">
              <p className="text-[11px] font-semibold text-red-400">Unrecoverable</p>
      ...
      <div className="px-4 pt-4 pb-2 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-zinc-100">
            Ghost Recovery <span className="text-base">👻</span>
          </h2>
      ...
      <button onClick={onTap} className="w-full text-left">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className={`text-[11px] font-semibold ${isExpired ? "text-zinc-500 italic" : "text-zinc-400 italic"}`}>
      ...
      {!isExpired && (
        <div className="flex items-center justify-between mt-2">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg text-[10px] font-semibold bg-evap-green text-black hover:bg-evap-green/90 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
function ProofStatusBanner({ status }: { status: "valid" | "expiring" | "expired" }) {
  ...
  return (
    <div className={`px-3 py-2 rounded-lg ${c.bg} border ${c.border}`}>
      <p className={`text-[11px] font-medium ${c.text} text-center`}>
        {c.icon} {c.label}
      </p>
    </div>
  );
...
function MetaRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`text-[10px] text-zinc-300 ${mono ? "font-mono" : ""}`}>{value}</span>
    </div>
  );
}
