    ...
    return (
      <div className="rounded-lg border border-evap-border bg-evap-surface p-3">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Transaction Preview</span>
        </div>
        ...
        {simError && (
          <div className="rounded-md bg-evap-red/10 border border-evap-red/30 px-3 py-2 mb-3">
            <p className="text-xs text-evap-red">{simError}</p>
          </div>
        )}
        ...
        <div className="flex gap-2">
          <button
            onClick={onCancel}
            className="flex-1 py-2 rounded-lg border border-evap-border text-xs text-zinc-400 hover:text-zinc-200 hover:border-zinc-500 transition"
          >
            Cancel
          </button>
          <button
            onClick={handleSimulate}
            className="flex-1 py-2 rounded-lg bg-evap-cyan/20 border border-evap-cyan/30 text-xs text-evap-cyan font-medium hover:bg-evap-cyan/30 transition"
          >
            Simulate
          </button>
        </div>
  ...
  return (
    <div className="rounded-lg border border-evap-border bg-evap-surface overflow-hidden">
      ...
      <div className="px-3 py-2 border-b border-evap-border bg-evap-bg/50">
        <span className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Simulation Result</span>
      </div>
      ...
      <div className="p-3 space-y-3">
        ...
        {warnings.map((w, i) => (
          ...
          >
            <p className={`text-xs font-medium ${w.level === "red" ? "text-evap-red" : "text-evap-amber"}`}>
              {w.level === "red" ? "Warning" : "Caution"}
            </p>
            <p className={`text-xs mt-0.5 ${w.level === "red" ? "text-evap-red/80" : "text-evap-amber/80"}`}>
              {w.message}
            </p>
          </div>
        ))}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between text-xs pt-1.5 border-t border-evap-border">
        ...
        {simulation!.objectSurvivalEpochs !== undefined && (
          <div className="flex justify-between items-center text-xs px-2 py-1.5 rounded bg-evap-bg border border-evap-border">
...
{JSON.stringify(
  ...
  2
        ...
        <div className="flex gap-2 pt-1">
          <button
            onClick={onCancel}
            className="flex-1 py-2.5 rounded-lg border border-evap-border text-xs text-zinc-400 hover:text-zinc-200 hover:border-zinc-500 transition"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 py-2.5 rounded-lg bg-gradient-to-r from-evap-cyan to-evap-purple text-xs font-semibold text-black hover:opacity-90 transition disabled:opacity-40"
          >
            Confirm & Send
          </button>
        </div>
