...
mod tests {
    ...
    #[test]
    fn test_import_account_with_address_duplicate_covers_line_182() {
        use evaporchain_crypto::signatures::{MlDsaKeypair, Signer};
        let mut mgr = make_manager();
        mgr.create_account("alice", "pass").unwrap();
        let kp = MlDsaKeypair::generate();
        let pk = kp.public_key_bytes();
        let sk = kp.secret_key().to_vec();
        // Duplicate name → import_key_with_address errors → ? propagates
        let result = mgr.import_account_with_address("alice", "pass2", &pk, &sk, [0xABu8; 32]);
        assert!(result.is_err());
    }

    #[test]
    fn test_remove_nonactive_account_covers_lines_204_205() {
        let mut mgr = make_manager();
        mgr.create_account("alice", "pass1").unwrap(); // alice is active
        mgr.create_account("bob", "pass2").unwrap();
        // Remove bob (NOT the active account) → active stays alice
    ...
    #[test]
    fn test_increment_nonce_unknown_name_covers_line_315() {
        let mut mgr = make_manager();
        // Name not in keystore → outer if-let-Some(addr) is None → skip body
        mgr.increment_nonce("nobody");
    }
}
