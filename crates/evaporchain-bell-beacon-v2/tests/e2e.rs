...
use evaporchain_bell_beacon_v2::{
    issue_certificate, verify_certificate, BeaconError, BellCertificate, ConcurrentPair, PairStats,
    VerifyError,
};
...
fn window1_cert_passes_gate_and_verifies() {
    ...
    assert!(
        cert.s_cartel_milli > cert.s_honest_milli,
...
fn three_window_chain_seeds_are_distinct() {
    ...
    assert_ne!(
        cert1.seed, cert2.seed,
        "cert1 and cert2 seeds must differ (different prevs)"
    );
...
fn doctrine_pair_reordering_does_not_change_seed() {
    ...
    let pairs = balanced_pairs();
    let cert_fwd =
        issue_certificate(CHAIN_MAINNET, 100, 200, &pairs, thresholds(), GENESIS_PREV).unwrap();

    let mut pairs_rev = pairs.clone();
    pairs_rev.reverse();
    let cert_rev = issue_certificate(
        CHAIN_MAINNET,
        100,
        200,
        &pairs_rev,
...
fn doctrine_bft_determinism_same_inputs_yield_identical_certs() {
    ...
    assert_eq!(
        cert_v1, cert_v2,
        "same inputs must produce byte-identical certificates"
    );
}
...
fn adversarial_cross_chain_replay_rejected() {
    ...
    let pairs = balanced_pairs();
    let testnet_cert =
        issue_certificate(CHAIN_TESTNET, 100, 200, &pairs, thresholds(), GENESIS_PREV).unwrap();
...
fn adversarial_tampered_seed_rejected() {
    ...
    let err =
        verify_certificate(CHAIN_MAINNET, &pairs, GENESIS_PREV, thresholds(), &cert).unwrap_err();
    assert_eq!(
        err,
        VerifyError::SeedMismatch,
        "tampered seed must be rejected"
    );
}
...
fn adversarial_tampered_prev_hash_rejected() {
    ...
    let err =
        verify_certificate(CHAIN_MAINNET, &pairs, [0xFFu8; 32], thresholds(), &cert).unwrap_err();
    assert_eq!(
        err,
        VerifyError::PrevHashMismatch,
...
fn adversarial_tampered_s_honest_rejected() {
    ...
    let err =
        verify_certificate(CHAIN_MAINNET, &pairs, GENESIS_PREV, thresholds(), &cert).unwrap_err();
    assert!(
        matches!(err, VerifyError::SHonestMismatch { .. }),
...
fn adversarial_empty_window_rejected_at_issuance() {
    let err =
        issue_certificate(CHAIN_MAINNET, 100, 200, &[], thresholds(), GENESIS_PREV).unwrap_err();
    assert_eq!(
        err,
        BeaconError::EmptyWindow,
        "empty window must fail at issuance"
    );
}
...
fn adversarial_single_bucket_window_rejected() {
    ...
    let single_bucket: Vec<ConcurrentPair> = (0u8..8)
        .map(|i| {
            let mut tag = [0u8; 32];
    ...
    assert_eq!(
        err,
        BeaconError::InsufficientCoverage,
...
fn adversarial_invalid_window_range_rejected() {
    ...
    assert!(
        matches!(
            err,
            BeaconError::InvalidWindowRange {
                start: 200,
                end: 100
            }
        ),
...
fn adversarial_prev_hash_in_chain_linkage_rejected() {
    ...
    let err =
        verify_certificate(CHAIN_MAINNET, &pairs, GENESIS_PREV, thresholds(), &cert2).unwrap_err();
    assert_eq!(
        err,
        VerifyError::PrevHashMismatch,
