...
fn gdpr_platform_three_record_lifecycle() {
    ...
    assert_eq!(
        alice_proof.decayed_commitment, 31_250,
    ...
    assert_eq!(
        bob_not_yet.decayed_commitment, 500_000,
        "Bob at epoch 100: 1 halving of 1_000_000 must yield 500_000"
    );
    assert!(
        verify_forget_proof(&bob_not_yet).is_err(),
    ...
    assert_eq!(
        bob_forgotten.decayed_commitment, 125_000,
    ...
    assert_eq!(
        carol_proof.decayed_commitment, 0,
...
fn different_query_epochs_produce_different_decayed_values() {
    ...
    assert!(
        p_100.decayed_commitment > p_200.decayed_commitment,
        "decayed commitment must decrease as query epoch advances"
    );
    assert!(
        p_200.decayed_commitment > p_500.decayed_commitment,
...
fn tamper_any_proof_field_invalidates_witness() {
    ...
    let tampers: Vec<(&str, DecayForgetProof)> = vec![
        ...
        ("forgotten_at_epoch", {
            let mut p = proof.clone();
            p.forgotten_at_epoch += 1;
            p
        }),
        ("forget_threshold", {
            let mut p = proof.clone();
            p.forget_threshold += 1;
            p
        }),
        ("decayed_commitment", {
            let mut p = proof.clone();
            p.decayed_commitment += 1;
            p
        }),
        ("witness_direct", {
            let mut p = proof.clone();
...
fn adversarial_raised_threshold_in_proof_invalidates_witness() {
    ...
    assert!(
        verify_forget_proof(&not_forgotten).is_err(),
...
fn activated_after_query_elapsed_zero_so_full_commitment_not_forgotten() {
    ...
    assert_eq!(
        proof.decayed_commitment, 1_000_000,
        "activated after query → zero elapsed → full commitment"
    );
    let err = verify_forget_proof(&proof).unwrap_err();
    assert!(
        matches!(err, ForgetProofError::NotForgotten { .. }),
