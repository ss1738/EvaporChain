...
impl PrivacyExecutor {
    ...
    pub fn execute_unshield(
        ...
        // PRIV-N3: check input-amount sum BEFORE nullifier/anchor checks so
        // BalanceOverflow is returned before DoubleSpend on crafted txs.
        let sum_in: u64 = tx
            .input_amounts
            .iter()
        ...
        // PRIV-N3: check input-amount sum BEFORE nullifier/anchor checks.
        let sum_in: u64 = tx
            .input_amounts
            .iter()
            .try_fold(0u64, |acc, &x| acc.checked_add(x))
        ...
        // (sum_in computed above before nullifier checks — PRIV-N3)
