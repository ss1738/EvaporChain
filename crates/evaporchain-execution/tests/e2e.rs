...
use evaporchain_execution::{
    evaluate_conservation_gate, ExecutionEngine, ExecutionError,
    GAS_TRANSFER, SimpleExecutor,
};
...
fn gate_err_observe_is_ok_err() {
    let violation = ConservationViolation::RedirectChangedTotal { before: 1000, after: 999 };
    let result = evaluate_conservation_gate(Err(violation), false);
    assert!(
        matches!(result, Ok(Err(ConservationViolation::RedirectChangedTotal { .. }))),
...
fn gate_err_enforce_is_execution_error() {
    let violation = ConservationViolation::RedirectChangedTotal { before: 2000, after: 1999 };
...
fn empty_block_succeeds() {
    ...
    let result = exec.execute_block(&mut db, &make_block(1, 1, vec![])).unwrap();
...
fn funded_transfer_executes_successfully() {
    ...
    let result = exec
        .execute_block(&mut db, &make_block(1, 1, vec![transfer_tx(0xAA, 0xBB, 500, 0)]))
        .unwrap();
...
fn unfunded_transfer_is_counted_in_txs_failed() {
    ...
    let result = exec
        .execute_block(&mut db, &make_block(1, 1, vec![transfer_tx(0x01, 0x02, 999_999, 0)]))
        .unwrap();
...
fn funded_transfer_moves_balance() {
    ...
    exec.execute_block(&mut db, &make_block(1, 1, vec![transfer_tx(0xAA, 0xBB, 300, 0)])).unwrap();
...
fn sequential_blocks_produce_distinct_state_roots() {
    ...
    let r1 = exec.execute_block(&mut db, &make_block(1, 1, vec![transfer_tx(0xAA, 0xBB, 100, 0)])).unwrap();
    let r2 = exec.execute_block(&mut db, &make_block(2, 2, vec![transfer_tx(0xAA, 0xBB, 100, 1)])).unwrap();
...
fn zero_tx_block_is_idempotent_state_root() {
    ...
    let r1 = exec1.execute_block(&mut db1, &make_block(1, 1, vec![])).unwrap();

    let mut exec2 = SimpleExecutor::new(10);
    let mut db2   = InMemoryStateDB::new();
    let r2 = exec2.execute_block(&mut db2, &make_block(1, 1, vec![])).unwrap();
...
fn self_transfer_is_not_executed() {
    ...
    let result = exec
        .execute_block(&mut db, &make_block(1, 1, vec![transfer_tx(0xAA, 0xAA, 100, 0)]))
        .unwrap();
    assert_eq!(result.txs_failed, 1, "self-transfer must be counted as failed");
}
...
fn zero_amount_transfer_is_rejected() {
    ...
    let result = exec
        .execute_block(&mut db, &make_block(1, 1, vec![transfer_tx(0xAA, 0xBB, 0, 0)]))
        .unwrap();
    assert_eq!(result.txs_failed, 1, "zero-amount transfer must be counted as failed");
}
...
fn elena_execution_full_arc() {
    ...
    let r1 = exec.execute_block(&mut db, &make_block(1, 1, vec![
        transfer_tx(kaito,  rafael, 1_000, 0),
        transfer_tx(rafael, igor,   500,   0),
    ])).unwrap();

    assert_eq!(r1.txs_executed, 2, "both funded txs must execute in block 1");
    assert_eq!(r1.txs_failed,   0, "no failed txs in block 1");

    // Block 2: IGOR transfers back 200 to RAFAEL.
    let r2 = exec.execute_block(&mut db, &make_block(2, 2, vec![
        transfer_tx(igor, rafael, 200, 0),
    ])).unwrap();
    ...
    assert_ne!(r1.state_root, r2.state_root, "state root must change between blocks");
    ...
    let v = ConservationViolation::RedirectChangedTotal { before: 1, after: 0 };
    let gate_obs = evaluate_conservation_gate(Err(v), false);
    assert!(matches!(gate_obs, Ok(Err(_))), "observe mode must allow commit despite violation");

    // Conservation gate: same violation in enforce mode must reject.
    let v2 = ConservationViolation::RedirectChangedTotal { before: 1, after: 0 };
    let gate_enf = evaluate_conservation_gate(Err(v2), true);
    assert!(matches!(gate_enf, Err(ExecutionError::ConservationViolation(_))),
