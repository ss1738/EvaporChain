...
mod tests {
    ...
    // ─── Boundary / quiescent paths ──────────────────────────────────────

    #[test]
    fn test_start_round_increments_counter_monotonically() {
        let mut bridge = OracleBridge::new(1);
        let r1 = bridge.start_round("btc_usd");
        let r2 = bridge.start_round("eth_usd");
        let r3 = bridge.start_round("sol_usd");
        assert_eq!(r1, 1);
        assert_eq!(r2, 2);
        assert_eq!(r3, 3);
        assert_eq!(bridge.active_rounds_count(), 3);
    }

    #[test]
    fn test_get_value_and_twap_unknown_key_returns_none() {
        let bridge = OracleBridge::new(1);
        assert!(bridge.get_value("never_seen").is_none());
        assert!(bridge.get_twap("never_seen").is_none());
        assert!(bridge.generate_proof("never_seen").is_none());
        assert_eq!(bridge.feed_count(), 0);
    }

    #[test]
    fn test_submit_vote_without_active_round_errors() {
        let mut bridge = OracleBridge::new(1);
        let kp = BlsKeypair::generate();
        let (v, pk) = signed(&kp, 0, "no_round", 1.0, 0, 0);
        let err = bridge.submit_vote("no_round", v, &pk).unwrap_err();
        assert!(err.contains("no active round"), "{err}");
    }

    #[test]
    fn test_try_finalize_unknown_key_returns_none() {
        let mut bridge = OracleBridge::new(1);
        assert!(bridge.try_finalize("never_started").is_none());
    }

    #[test]
    fn test_try_finalize_without_quorum_returns_none() {
        // Quorum is 2, only one vote submitted ⇒ has_quorum() is false.
        let mut bridge = OracleBridge::new(2);
        let r = bridge.start_round("btc_usd");
        let kp = BlsKeypair::generate();
        let (v, pk) = signed(&kp, 0, "btc_usd", 60000.0, r, 1000);
        bridge.submit_vote("btc_usd", v, &pk).unwrap();
        assert!(bridge.try_finalize("btc_usd").is_none());
        // Round still active — wasn't removed.
        assert_eq!(bridge.active_rounds_count(), 1);
    }
}
