...
fn main() {
    ...
    let cf_names = DB::list_cf(&Options::default(), &chain_path)
        .unwrap_or_else(|_| vec!["default".to_string(), CF_META.to_string()]);
    ...
    if mode == "write" {
        ...
        if hex_str.len() != 64 {
            eprintln!(
                "parent_hash must be 64 hex chars (32 bytes), got {}",
                hex_str.len()
            );
            std::process::exit(1);
        }
        ...
        db.put_cf(cf, b"parent_hash", &new_hash)
            .unwrap_or_else(|e| {
