...
mod tests {
    ...
    #[test]
    fn test_session_authenticate_accepts_raw_token_without_bearer_prefix() {
        // The handler strips "Bearer " when present but accepts a raw token too —
        // some old clients send the token alone. Lock that contract.
        let sessions: Sessions = Arc::new(Mutex::new(HashMap::new()));
        let token = "raw_token_no_bearer";
        sessions
            .lock()
            .unwrap()
            .insert(token.to_string(), (7, Instant::now()));
        let mut headers = HeaderMap::new();
        headers.insert("authorization", token.parse().unwrap());
        assert_eq!(authenticate(&headers, &sessions).unwrap(), 7);
    }

    #[test]
    fn test_token_uniqueness_across_calls() {
        // 64 random bytes → astronomically unlikely to collide; lock that the
        // generator uses fresh entropy and not a constant.
        ...
        for t in [&t1, &t2, &t3] {
            assert_eq!(t.len(), 128);
            assert!(hex::decode(t).is_ok(), "token must be valid hex");
        }
    }

    #[test]
    fn test_verification_code_in_six_digit_range() {
        // 100_000..999_999 — never zero-prefixed, always 6 digits.
        for _ in 0..50 {
            let code = generate_verification_code();
            assert_eq!(code.len(), 6);
    ...
    #[test]
    fn test_master_encryption_key_is_deterministic_for_given_env() {
        // Without EVAPORCHAIN_KEY_MASTER set, the dev fallback is deterministic.
        // Two calls in the same process must yield the same key.
        let k1 = master_encryption_key();
        let k2 = master_encryption_key();
        assert_eq!(k1, k2);
        // 32 bytes, blake3-derived ⇒ not all zero.
        assert_ne!(k1, [0u8; 32]);
    }

    #[test]
    fn test_address_derivation_deterministic() {
        let pk = b"some-deterministic-pubkey-bytes";
