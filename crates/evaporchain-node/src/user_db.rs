...
impl UserDb {
    ...
    pub fn get_user_by_email(
        &self,
        email: &str,
    ) -> Result<Option<(i64, String, String, bool, Option<String>)>, String> {
        let conn = self.conn.lock().unwrap();
    ...
    pub fn get_wallet_owner(&self, address: &str) -> Result<Option<i64>, String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT user_id FROM wallets WHERE address = ?1")
            .map_err(|e| format!("{e}"))?;
    ...
    pub fn get_wallet_keys(&self, address: &str) -> Result<Option<(String, String)>, String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT public_key, encrypted_private_key FROM wallets WHERE address = ?1")
            .map_err(|e| format!("{e}"))?;
...
mod tests {
    ...
    fn test_create_user() {
        let db = temp_db();
        let id = db
            .create_user("test@example.com", "$hash", "Test User", "123456")
            .unwrap();
        assert!(id > 0);
    }
    ...
    fn test_duplicate_email_fails() {
        let db = temp_db();
        db.create_user("dup@example.com", "$hash", "User1", "111111")
            .unwrap();
    ...
    fn test_get_user_by_email() {
        let db = temp_db();
        db.create_user("find@example.com", "$hash", "Findme", "999999")
            .unwrap();
    ...
    fn test_verify_email() {
        let db = temp_db();
        db.create_user("verify@example.com", "$h", "V", "654321")
            .unwrap();
    ...
    fn test_create_and_list_wallets() {
        let db = temp_db();
        let uid = db
            .create_user("wallet@example.com", "$h", "W", "000000")
            .unwrap();
        db.create_wallet(uid, "Main", "0xabc123", "pub123", "enc_priv123")
            .unwrap();
        db.create_wallet(uid, "Trading", "0xdef456", "pub456", "enc_priv456")
            .unwrap();
    ...
    fn test_wallet_owner() {
        let db = temp_db();
        let uid = db
            .create_user("own@example.com", "$h", "O", "000000")
            .unwrap();
        db.create_wallet(uid, "Mine", "0xowner", "pub", "enc")
            .unwrap();
    ...
    fn test_activity_log() {
        let db = temp_db();
        let uid = db
            .create_user("act@example.com", "$h", "A", "000000")
            .unwrap();
        db.log_activity(uid, "0xaddr", "transfer", "Sent 500 EVAP")
            .unwrap();
        db.log_activity(uid, "0xaddr", "faucet", "Claimed 10000 EVAP")
            .unwrap();
    ...
    fn test_wallet_stores_encrypted_private_key() {
        let db = temp_db();
        let uid = db
            .create_user("enc@example.com", "$h", "E", "000000")
            .unwrap();
        let encrypted_key = "aabbccdd_encrypted_blob";
        db.create_wallet(uid, "Secure", "0xsecure", "pub_key", encrypted_key)
            .unwrap();
        let keys = db.get_wallet_keys("0xsecure").unwrap().unwrap();
        assert_eq!(keys.0, "pub_key");
        assert_eq!(
            keys.1, encrypted_key,
            "stored key must be the encrypted blob, not plaintext"
        );
    }

    #[test]
    fn test_get_user_by_id_unknown_returns_none() {
        let db = temp_db();
        assert!(db.get_user_by_id(99999).unwrap().is_none());
    }

    #[test]
    fn test_get_user_by_id_returns_full_user() {
        let db = temp_db();
        let id = db
            .create_user("byid@example.com", "$h", "ByName", "000000")
            .unwrap();
        let user = db.get_user_by_id(id).unwrap().unwrap();
        assert_eq!(user.id, id);
        assert_eq!(user.email, "byid@example.com");
        assert_eq!(user.display_name, "ByName");
        assert!(!user.email_verified);
    }

    #[test]
    fn test_list_wallets_empty_user_returns_empty() {
        let db = temp_db();
        let uid = db
            .create_user("empty@example.com", "$h", "Empty", "000000")
            .unwrap();
        let wallets = db.list_wallets(uid).unwrap();
    ...
    #[test]
    fn test_get_activity_unknown_user_returns_empty() {
        let db = temp_db();
        let activity = db.get_activity(99999, 10).unwrap();
        assert!(activity.is_empty());
    }

    #[test]
    fn test_get_wallet_keys_unknown_address_returns_none() {
        let db = temp_db();
        assert!(db.get_wallet_keys("0xnope").unwrap().is_none());
    }

    #[test]
    fn test_create_wallet_duplicate_address_fails() {
        let db = temp_db();
        let uid = db
            .create_user("dupw@example.com", "$h", "U", "000000")
            .unwrap();
        db.create_wallet(uid, "First", "0xshared", "pk1", "enc1")
            .unwrap();
        // Second wallet with the same address must fail (UNIQUE constraint).
        let err = db.create_wallet(uid, "Second", "0xshared", "pk2", "enc2");
        assert!(err.is_err());
    }

    #[test]
    fn test_get_activity_respects_limit() {
        let db = temp_db();
        let uid = db
            .create_user("limit@example.com", "$h", "L", "000000")
            .unwrap();
        for i in 0..5 {
            db.log_activity(uid, "0xa", "act", &format!("event {i}"))
                .unwrap();
        }
        let two = db.get_activity(uid, 2).unwrap();
        assert_eq!(two.len(), 2);
        let all = db.get_activity(uid, 100).unwrap();
        assert_eq!(all.len(), 5);
    }
}
