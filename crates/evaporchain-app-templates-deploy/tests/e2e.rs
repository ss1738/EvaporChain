...
use evaporchain_app_templates::class::{MAYFLY, SDDC_AUCTION, SINGH_SABI, MNEMOCHAIN_CARD};
...
fn domain_separation_prevents_cross_template_replay() {
    ...
    assert_ne!(mayfly.signing_bytes().unwrap(), sabi.signing_bytes().unwrap(),
...
fn nonce_change_produces_different_commitment() {
    ...
    assert_ne!(r0.commitment().unwrap(), r1.commitment().unwrap(),
...
fn arrays_preserve_element_order_in_commitment() {
    ...
    assert_ne!(r1.commitment().unwrap(), r2.commitment().unwrap(),
...
fn nadia_portfolio_full_arc() {
    ...
    let deploys = [
        DeployRequest::new(SINGH_SABI,
            json!({"initial_energy": 5_000, "floor_pct": 20, "half_life": 365}),
            nadia(), 0, 0).unwrap(),
        mayfly_req(nadia(), 1),
        DeployRequest::new(SDDC_AUCTION,
            json!({"ceiling": 1_000, "floor": 100, "lot_lambda": 50, "duration_epochs": 500}),
            nadia(), 0, 2).unwrap(),
        DeployRequest::new(MNEMOCHAIN_CARD,
            json!({"initial_energy": 1_000, "initial_stability": 10}),
            nadia(), 0, 3).unwrap(),
    ];
    let commitments: Vec<_> = deploys.iter()
        .map(|r| r.commitment().unwrap())
        .collect();
    let unique: std::collections::HashSet<_> = commitments.iter().collect();
    assert_eq!(unique.len(), 4, "all four deploys must have distinct commitments");
}
