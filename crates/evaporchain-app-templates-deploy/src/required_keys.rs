...
use evaporchain_app_templates::{
    class::{
        BELL_ORACLE, CHILDKEY_LETTER, DEADMAN_SWITCH, DECAY_ACCESS_PASS, ERASURE_ATTESTATION,
        EVAPORCASH_NOTE, GALLERY_FORGETS, GDPR_VAULT, LOTTERY_CLASS, MAYFLY, MNEMOCHAIN_CARD,
        MORTAL_DAO, MORTAL_MESSAGE_PILOT, MORTAL_NFT_GENERAL, MULTISIG_PROPOSAL, OPEN_BOUNTY,
        ORACLE_FEED, PAYMENT_SPLIT_CLASS, REFRESH_MARKET_NAMESPACE, SAP_AQ, SBAV_RUNTIME,
        SCL_LEASE, SDDC_AUCTION, SEALED_BID_AUCTION_CLASS, SFSV_VAULT, SGB_TYPE_SYSTEM,
        SHLM_BOUNTY, SINGH_HEARTBEAT_PULSE, SINGH_LINEAGE_POLICY, SINGH_MIGRANT, SINGH_POSTHUMA,
...
pub fn required_keys_for(class: TemplateClass) -> &'static [&'static str] {
    match class {
        ...
        c if c == PAYMENT_SPLIT_CLASS => &["initial_energy", "half_life"],
        c if c == SEALED_BID_AUCTION_CLASS => &["initial_energy", "half_life", "default_reserve_price"],
        c if c == LOTTERY_CLASS => &["initial_energy", "half_life", "default_prize", "default_stake"],
        // ── Wallet UX lane ────────────────────────────────────────
        ...
        c if c == MULTISIG_PROPOSAL => &["initial_energy", "half_life", "default_threshold"],
        // ── Consumer lane ─────────────────────────────────────────
        ...
        c if c == ERASURE_ATTESTATION => {
            &["initial_energy", "half_life", "default_obligation_basis", "default_method"]
        }
        c if c == MORTAL_DAO => &[
            "energy",
            "half_life",
