...
mod tests {
    ...
    #[test]
    fn test_sample_queries_deterministic_under_same_seed() {
        let da = BlockDA::new().unwrap();
        let package = da.encode_block(b"determinism check").unwrap();
        let q1 = BlockDA::generate_sample_queries(7, &package.header, 16, b"sX");
        let q2 = BlockDA::generate_sample_queries(7, &package.header, 16, b"sX");
    ...
    #[test]
    fn test_verify_with_wrong_commitment_root_fails() {
        let da = BlockDA::new().unwrap();
        let package = da.encode_block(b"commit root mismatch").unwrap();
        let response = da.prove_shard(&package, 2).unwrap();
        let mut header = package.header.clone();
        header.commitment_root[0] ^= 0xFF;
        assert!(!BlockDA::verify_shard_sample(&header, &response));
    }

    #[test]
    fn test_verify_with_corrupted_shard_hash_fails() {
        let da = BlockDA::new().unwrap();
        let package = da.encode_block(b"hash mismatch on shard").unwrap();
        let mut response = da.prove_shard(&package, 0).unwrap();
        // Tamper hash without touching data — first check (`computed_hash !=
        // shard.hash`) must catch this.
        response.shard.hash[0] ^= 0x01;
        assert!(!BlockDA::verify_shard_sample(&package.header, &response));
    }

    #[test]
    fn test_encode_block_data_hash_matches_blake3() {
        let da = BlockDA::new().unwrap();
        let block_data = b"locked hash invariant";
        let package = da.encode_block(block_data).unwrap();
        let expected: [u8; 32] = blake3::hash(block_data).into();
        assert_eq!(package.header.data_hash, expected);
    }

    #[test]
    fn test_with_custom_config_records_config_in_header() {
        let cfg = ErasureConfig {
            data_shards: 6,
            parity_shards: 3,
        };
        let da = BlockDA::with_config(cfg).unwrap();
        let package = da.encode_block(b"custom config block").unwrap();
        assert_eq!(package.header.erasure_config.data_shards, 6);
        assert_eq!(package.header.erasure_config.parity_shards, 3);
        assert_eq!(package.header.total_shards, 9);
        assert_eq!(package.shards.len(), 9);
    }
}
