...
fn temperature_decays_monotonically() {
    ...
    for epoch in [100u64, 500, 1000, 5000, 10_000] {
        ...
        assert!(
            t <= prev,
            "T must monotonically decay: T({epoch})={t} > prev={prev}"
        );
        prev = t;
    }
}
...
fn always_accept_strict_improvement() {
    let p = params(1000);
    let old = AnnealedScore {
        stake: 100,
        activity: 1,
        uptime_milli: 500,
    };
    let new = AnnealedScore {
        stake: 1_000,
        activity: 10,
        uptime_milli: 1_000,
    };
...
fn different_nonces_can_differ_only_in_degrading_branch() {
    ...
    assert!(
        saw_reject,
        "SA must reject some nonces at degrading-move + low T"
    );
}
...
fn annealed_score_eq_and_copy() {
    let s1 = AnnealedScore {
        stake: 100,
        activity: 5,
        uptime_milli: 1_000,
    };
    let s2 = AnnealedScore {
        stake: 100,
        activity: 5,
        uptime_milli: 1_000,
    };
    let s3 = AnnealedScore {
        stake: 100,
        activity: 5,
        uptime_milli: 999,
    };
