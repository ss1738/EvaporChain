...
fn five_epoch_session_in_basin_and_fallback() {
    ...
    assert_eq!(
        e1.selected_center, LOW_CENTER,
        "E1: LOW-traffic state must pick LOW regime"
    );
    assert!(!e1.used_fallback, "E1: in-basin must not use fallback");
    assert!(
        e1.drift > 0,
        "E1: state below center, drift must be positive"
    );
    ...
    assert_eq!(
        e2.selected_center, MID_CENTER,
        "E2: active-DEX state must pick MID regime"
    );
    assert!(!e2.used_fallback);
    assert!(
        e2.drift < 0,
        "E2: state above center, drift must be negative"
    );
    ...
    assert!(
        e4.used_fallback,
        "E4: gap state must use Bell-seed fallback"
    );
    ...
    assert!(
        e5.used_fallback,
        "E5: post-HIGH state must use Bell-seed fallback"
    );
...
fn drift_direction_and_magnitude_correct() {
    ...
    assert_eq!(
        below.drift, 50,
        "state below center: drift must be +drift_rate"
    );
    ...
    assert_eq!(
        above.drift, -50,
        "state above center: drift must be -drift_rate"
    );
    ...
    assert_eq!(
        close.drift, 5,
        "close state: drift clamped to distance (5), not rate (50)"
    );
}
...
fn adversarial_zero_basin_with_nonzero_drift_is_not_degenerate() {
    ...
    assert!(
        r.is_ok(),
        "zero-basin with drift_rate>0 must not be rejected"
    );
}
