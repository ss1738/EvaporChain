...
fn permissive_lambda_admits_all_nonzero_kl_validators() {
    ...
    for (name, sig) in [
        ...
        assert!(
            sig.kl_millibits(&prior) > 0,
            "{name} must have KL > 0 vs prior"
        );
        assert_eq!(
            ib_vote(&sig, &prior, &permissive),
            IbVote::Commit,
...
fn strict_lambda_excludes_low_divergence_validators() {
    ...
    assert!(
        kl_v2 < 3_500_000,
        "V2 KL ({kl_v2}) must be below strict threshold"
    );
    assert!(
        kl_v5 < 3_500_000,
        "V5 KL ({kl_v5}) must be below strict threshold"
    );
    assert_eq!(
        ib_vote(&v2_sig(), &prior, &strict),
        IbVote::Abstain,
        "V2 must ABSTAIN at strict λ"
    );
    assert_eq!(
        ib_vote(&v5_sig(), &prior, &strict),
        IbVote::Abstain,
        "V5 must ABSTAIN at strict λ"
    );
}
...
fn doctrine_larger_lambda_admits_fewer_validators() {
    ...
    let count = |params: IbParams| {
        sigs.iter()
            .filter(|(_, f)| ib_vote(&f(), &prior, &params) == IbVote::Commit)
            .count()
    };
...
fn adversarial_max_lambda_forces_all_abstain() {
    ...
    let prior = prior_sig();
    let max_strict = IbParams {
        lambda_mb: u64::MAX,
    };

    for (name, f) in [
        ("V1", v1_sig as fn() -> StateSignature),
        ("V2", v2_sig),
        ("V3", v3_sig),
        ("V4", v4_sig),
    ] {
...
fn adversarial_zero_scale_signature_concentrates_in_bin_zero() {
    ...
    assert_eq!(
        sig.bins[0], 1_000,
        "zero-scale: all weight must land in bin 0"
    );
    for &b in &sig.bins[1..] {
