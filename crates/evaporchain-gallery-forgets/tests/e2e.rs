...
fn tid(b: u8)  -> [u8; 32]       { [b; 32] }
fn eid(b: u8)  -> [u8; 32]       { [b; 32] }
...
fn disintegration_gallery() -> Gallery {
    ...
    let c = addr(CURATOR);
    g.deposit(c, eid(0xD1), addr(0xA1),
        MayflyToken::mint(tid(0xD1), addr(0xA1), DALI_INIT,     DALI_HL,     0).unwrap(), 0).unwrap();
    g.deposit(c, eid(0xC2), addr(0xA2),
        MayflyToken::mint(tid(0xC2), addr(0xA2), CLAUDE_INIT,   CLAUDE_HL,   0).unwrap(), 0).unwrap();
    g.deposit(c, eid(0xB3), addr(0xA3),
        MayflyToken::mint(tid(0xB3), addr(0xA3), BASINSKI_INIT, BASINSKI_HL, 0).unwrap(), 0).unwrap();
    g.deposit(c, eid(0xA4), addr(0xA4),
        MayflyToken::mint(tid(0xA4), addr(0xA4), ARTEMIS_INIT,  ARTEMIS_HL,  0).unwrap(), 0).unwrap();
    g
}
...
fn disintegration_show_open_closing_closed_lifecycle() {
    ...
    assert_eq!(g.status_at(2), GalleryStatus::Open, "epoch 2: DALI barely alive, still Open");
    assert_eq!(g.alive_count(2), 4);

    // Epoch 3: DALI dead (4 >> 3 = 0) — first exhibit falls.
    assert_eq!(g.status_at(DALI_ACTUAL), GalleryStatus::Closing,
    ...
    assert_eq!(g.status_at(ARTEMIS_ACTUAL), GalleryStatus::Closed,
...
fn thermodynamic_close_epoch_upper_bounds_actual_closure_for_every_exhibit() {
    ...
    assert_eq!(close, THERMO_CLOSE, "thermodynamic close = max projected cert epoch");
    ...
    for ex in &g.exhibits {
        ...
        assert!(ex.mayfly.is_dead(proj),
            "exhibit {:?} must be dead by its cert epoch {proj}", ex.id);
    }
}
...
fn mayfly_death_certificate_pre_issued_before_any_decay() {
    ...
    assert!(promised > 0, "projected death epoch must be strictly in the future");
...
fn ai_seed_full_lifecycle_one_to_zero() {
    ...
    g.deposit(addr(CURATOR), eid(0x10), addr(0xAA), m, 0).unwrap();
    ...
    assert_eq!(s2.seed, 0.25, "after two half-lives (epoch 20), seed = 0.25");

    let s_dead = runtime_seed(&g, 100);
    assert_eq!(s_dead.seed, 0.0, "after all energy gone, seed = 0.0 (silence)");

    // Monotone non-increasing.
    let checkpoints = [s0, s1, s2, s_dead];
    for w in checkpoints.windows(2) {
        assert!(w[1].seed <= w[0].seed,
            "AI seed must be monotone non-increasing: {} → {}", w[0].seed, w[1].seed);
    }
}
...
fn dead_mayfly_blocks_transfer_alive_permits() {
    ...
    let err = dead.transfer(addr(0xAA), addr(0xBB), DALI_ACTUAL).unwrap_err();
    assert!(matches!(err, MayflyError::Died),
        "transfer of a dead Mayfly must yield Died error");

    // Owner is unchanged after the failed transfer.
    assert_eq!(dead.owner, addr(0xAA),
...
fn tampered_certificate_rejected_at_gallery_deposit() {
    ...
    assert!(!m.verify_certificate(), "tampered cert must fail verification");

    let err = g.deposit(addr(CURATOR), eid(0x01), addr(0xAA), m, 0).unwrap_err();
    assert!(matches!(err, GalleryError::InvalidCertificate),
...
fn non_curator_deposit_access_control() {
    ...
    let err = g.deposit(addr(0xBB), eid(0x01), addr(0xAA), m, 0).unwrap_err();
...
fn ai_seed_energy_weighted_big_exhibit_dominates() {
    ...
    g.deposit(addr(CURATOR), eid(0x01), addr(0xAA), giant, 0).unwrap();
    g.deposit(addr(CURATOR), eid(0x02), addr(0xBB), tiny,  0).unwrap();
...
fn two_galleries_have_independent_thermodynamic_clocks() {
    ...
    ga.deposit(addr(0xCA), eid(0xA1), addr(0xAA),
        MayflyToken::mint(tid(0xA1), addr(0xAA), DALI_INIT, DALI_HL, 0).unwrap(), 0).unwrap();

    let mut gb = Gallery::open(tid(0xB0), "Gallery B", addr(0xCB), 0);
    gb.deposit(addr(0xCB), eid(0xB1), addr(0xBB),
        MayflyToken::mint(tid(0xB1), addr(0xBB), ARTEMIS_INIT, ARTEMIS_HL, 0).unwrap(), 0).unwrap();
    ...
    assert_eq!(gb.status_at(close_a), GalleryStatus::Open,
    ...
    assert_eq!(ga.status_at(close_b), GalleryStatus::Closed,
...
fn gallery_status_is_deterministic_for_same_epoch() {
    let g = disintegration_gallery();
    for epoch in [0u64, 2, DALI_ACTUAL, CLAUDE_ACTUAL, BASINSKI_ACTUAL,
                  ARTEMIS_ACTUAL, THERMO_CLOSE] {
...
fn provably_mortal_single_sentence_doctrine() {
    ...
    assert_eq!(m.score_at(0), BASINSKI_INIT);
    assert!(m.verify_certificate(), "certificate verifiable at mint time");
    assert!(death_epoch > 0);

    // 3: provably dead at certified epoch.
    assert!(m.is_dead(death_epoch), "token provably dead at cert epoch {death_epoch}");
...
fn status_progression_open_then_closing_then_closed_never_reverses() {
    ...
    for &s in &statuses {
        match s {
            GalleryStatus::Open => {
                assert!(!seen_closing && !seen_closed,
                    "Open must not appear after Closing or Closed");
            }
            GalleryStatus::Closing => {
                seen_closing = true;
                assert!(!seen_closed,
                    "Closing must not appear after Closed");
            }
