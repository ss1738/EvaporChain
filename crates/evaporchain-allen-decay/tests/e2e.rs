...
use evaporchain_allen_decay::{compute_relation, Interval, IntervalError, AllenRelation};

// ── Fixture helpers ───────────────────────────────────────────────────────────

fn iv(s: u64, e: u64) -> Interval { Interval::new(s, e).unwrap() }

fn genesis()    -> Interval { iv(0,             1_000) }
fn warmup()     -> Interval { iv(1_000,        50_000) }
fn active()     -> Interval { iv(40_000,      900_000) }
fn flash_loan() -> Interval { iv(50_000,      200_000) }
fn governance() -> Interval { iv(400_000,     700_000) }
fn gov_wider()  -> Interval { iv(400_000,     800_000) }
fn gov_later()  -> Interval { iv(500_000,     700_000) }
fn gov_copy()   -> Interval { iv(400_000,     700_000) }
fn gov_sub()    -> Interval { iv(450_000,     650_000) }
fn audit()      -> Interval { iv(800_000,     950_000) }
fn shutdown()   -> Interval { iv(900_000,   1_100_000) }
fn tail()       -> Interval { iv(1_100_001, 1_200_000) }
...
fn all_13_allen_relations_exercised_in_defi_lifecycle() {
    ...
    assert_eq!(compute_relation(genesis(), flash_loan()), AllenRelation::Before,
    ...
    assert_eq!(compute_relation(active(), governance()), AllenRelation::Contains,
    ...
    assert_eq!(compute_relation(gov_wider(), governance()), AllenRelation::StartedBy,
    ...
    assert_eq!(compute_relation(audit(), active()), AllenRelation::OverlappedBy,
        "audit [800k,950k) must be OverlappedBy active [40k,900k)");

    // 12. MetBy — shutdown starts exactly where active ends (both at 900_000).
    assert_eq!(compute_relation(shutdown(), active()), AllenRelation::MetBy,
    ...
    assert_eq!(compute_relation(tail(), shutdown()), AllenRelation::After,
...
fn pair_flip_inversion_holds_for_every_lifecycle_pair() {
    ...
    let intervals = [
        genesis(), warmup(), active(), flash_loan(),
        governance(), gov_wider(), gov_later(), audit(), shutdown(), tail(),
    ];
    for &a in &intervals {
        for &b in &intervals {
...
fn symmetric_inverse_pairs_are_consistent() {
    ...
    for (r, inv) in pairs {
        assert_eq!(r.inverse(), inv,
            "{r:?}.inverse() must be {inv:?}");
        assert_eq!(inv.inverse(), r,
            "{inv:?}.inverse() must be {r:?}");
        assert_ne!(r, inv,
            "{r:?} must not equal its own inverse");
    }
...
fn adversarial_zero_width_interval_rejected() {
    ...
    assert!(matches!(err, IntervalError::EmptyOrInverted { start: 500, end: 500 }),
...
fn adversarial_inverted_interval_rejected() {
    ...
    assert!(matches!(err, IntervalError::EmptyOrInverted { start: 900_000, end: 400_000 }),
...
fn relation_compute_is_deterministic() {
    ...
    for _ in 0..100 {
        assert_eq!(compute_relation(active(), governance()), AllenRelation::Contains);
        assert_eq!(compute_relation(governance(), active()), AllenRelation::During);
    }
}
...
fn lifecycle_phase_chain_matches_intended_causal_order() {
    ...
    assert_eq!(compute_relation(warmup(), active()),      AllenRelation::Overlaps);
    assert_eq!(compute_relation(active(), flash_loan()),  AllenRelation::Contains);
    assert_eq!(compute_relation(active(), governance()),  AllenRelation::Contains);
...
fn governance_sub_window_relations_all_correct() {
    ...
    assert_eq!(compute_relation(gov_sub(), governance()), AllenRelation::During,
        "gov_sub must be During governance");
    assert_eq!(compute_relation(governance(), gov_sub()), AllenRelation::Contains,
    ...
    assert_eq!(compute_relation(gov_copy(), governance()), AllenRelation::Equals);
    assert_eq!(compute_relation(governance(), gov_copy()), AllenRelation::Equals);
}
