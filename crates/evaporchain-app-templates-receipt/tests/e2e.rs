...
fn canonical_bytes_start_with_domain_tag() {
    ...
    assert!(r.canonical_bytes().starts_with(RECEIPT_DOMAIN_TAG),
...
fn canonical_bytes_are_fixed_width() {
    ...
    assert_eq!(r.canonical_bytes().len(), RECEIPT_DOMAIN_TAG.len() + 124,
...
fn dapp_polls_receipt_by_commitment() {
    ...
    let r = receipt(req_commitment, [0x22; 32], MNEMOCHAIN_CARD, 1_250, 300, 5_002);
    assert_eq!(r.commitment, req_commitment,
...
fn serde_round_trip() {
    ...
    assert_eq!(r, back, "DeployReceipt must serialise and deserialise exactly");
}
...
fn nadia_batch_deploy_confirmation_arc() {
    ...
    let receipts: Vec<DeployReceipt> = (0..4).map(|i| {
        receipt(commitments[i], [i as u8 + 0x10; 32], classes[i], 1_200 + i as u64 * 100, 5_000, 5_000 + i as u64)
    }).collect();

    // All event IDs unique.
    let ids: std::collections::HashSet<_> = receipts.iter().map(|r| r.event_id()).collect();
    assert_eq!(ids.len(), 4, "all four receipts must have distinct event IDs");

    // Each receipt's commitment matches the original request commitment.
    for (i, r) in receipts.iter().enumerate() {
        assert_eq!(r.commitment, commitments[i], "receipt[{i}] commitment mismatch");
    }
}
