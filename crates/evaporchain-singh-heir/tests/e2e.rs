...
fn fixture_generation1_chain_decay_then_inherit_halves_decayed_value() {
    ...
    assert_eq!(
        n.energy, 1_500,
        "inheritance-tax halves chain-decayed 3_000 → 1_500"
    );
...
fn doctrine_active_vs_neglected_line_after_four_hops() {
    ...
    assert_eq!(
        neglected.energy, 1_000,
        "neglected: 16_000→8_000→4_000→2_000→1_000"
    );
    assert!(
        active.energy > neglected.energy * 10,
        "active line is 16× the neglected line"
    );
}
...
fn doctrine_kin_priority_ordering_is_deterministic() {
    ...
    assert_eq!(
        next,
        dan(),
        "first live heir (Dan, index 2) wins over Eve (index 3)"
    );
}
...
fn doctrine_mark_heir_state_changes_priority_outcome() {
    ...
    let mut n =
        HeirloomNft::mint(tid(8), alice(), 1_000, vec![live(dan()), live(eve())], 0).unwrap();
...
fn adversarial_self_loop_kin_rejected_in_set_kin() {
    ...
    assert_eq!(
        n.set_kin(vec![live(alice())]).unwrap_err(),
        HeirloomError::SelfLoopKin
    );
}
...
fn adversarial_escheated_token_blocks_all_mutations() {
    ...
    assert_eq!(
        n.mark_heir_state(bob(), HeirState::Live).unwrap_err(),
        HeirloomError::Escheated
    );
