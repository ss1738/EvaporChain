...
use evaporchain_app_templates_engine::{
    ...
    init_sbav, init_sddc, init_sgb, init_singh_heartbeat, init_singh_lineage,
    init_singh_posthuma, init_singh_sabi, init_sfsv, init_ssm, init_witnessfit,
    TypedInit,
};
...
fn mayfly()    -> TypedInit { TypedInit::Mayfly(init_mayfly::InitConfig { initial_energy: 1_000, half_life: 30 }) }
fn sabi()      -> TypedInit { TypedInit::SinghSabi(init_singh_sabi::InitConfig { initial_energy: 5_000, floor_pct: 15, half_life: 365 }) }
fn posthuma()  -> TypedInit { TypedInit::SinghPosthuma(init_singh_posthuma::InitConfig { half_life: 365, initial_visible_energy: 5_000, m_threshold: 3, n_committee: 5 }) }
fn sddc()      -> TypedInit { TypedInit::Sddc(init_sddc::InitConfig { ceiling: 1_000, floor: 100, lot_lambda: 50, duration_epochs: 500 }) }
fn mnemo()     -> TypedInit { TypedInit::Mnemochain(init_mnemochain::InitConfig { initial_energy: 1_000, initial_stability: 10 }) }
fn gallery()   -> TypedInit { TypedInit::GalleryForgets(init_gallery_forgets::InitConfig { opened_at_epoch: 0 }) }
fn heartbeat() -> TypedInit { TypedInit::SinghHeartbeat(init_singh_heartbeat::InitConfig { healthy_bpm: 60, alarmed_bpm: 120, amber_threshold_bp: 7500, red_threshold_bp: 5000 }) }
fn witnessfit()-> TypedInit { TypedInit::Witnessfit(init_witnessfit::InitConfig { half_life: 7, boost_bp: 500 }) }
fn sbav()      -> TypedInit { TypedInit::Sbav(init_sbav::InitConfig { reg_count: 4 }) }

fn lineage(n: usize) -> TypedInit {
    let rungs: Vec<_> = (0..n as u64).map(|i| init_singh_lineage::LadderRung {
        days: 30 * (i + 1), share_bp: 1000 * (i + 1),
    }).collect();
    TypedInit::SinghLineage(init_singh_lineage::InitConfig { ladder: rungs })
}

fn sgb(fragment: &str) -> TypedInit {
    TypedInit::Sgb(init_sgb::InitConfig { fragment: fragment.into() })
}

fn ssm(fragment: &str) -> TypedInit {
    TypedInit::Ssm(init_ssm::InitConfig { fragment: fragment.into() })
}

fn childkey() -> TypedInit {
    TypedInit::Childkey(init_childkey::InitConfig {
        unlock_age_years: 18, epochs_per_year: 365, m_threshold: 2, n_committee: 3,
    })
}

fn sfsv(addr: &str) -> TypedInit {
    TypedInit::Sfsv(init_sfsv::InitConfig {
        deposit_amount: 500, predicate_type: 0, release_param: 200, future_self: addr.into(),
    })
}
...
fn all_primitives_fee_at_or_above_base() {
    ...
    for typed in [
        mayfly(), sabi(), posthuma(), sddc(), mnemo(),
        gallery(), heartbeat(), witnessfit(), sbav(), childkey(),
        lineage(1), sgb("SLL"), ssm("game"),
    ] {
        let fee = fee_for(&typed);
        assert!(fee >= BASE_DEPLOY_FEE,
            "fee must be ≥ BASE_DEPLOY_FEE; got {fee} for {:?}", typed);
...
fn fee_is_deterministic_per_primitive() {
    ...
    for typed in [mayfly(), sabi(), sddc(), mnemo(), sbav()] {
        assert_eq!(fee_for(&typed), fee_for(&typed),
            "fee must be deterministic for {:?}", typed);
    }
}
...
fn lineage_fee_scales_linearly_with_ladder_rungs() {
    ...
    // Monotone.
    assert!(f1 < f3 && f3 < f5, "lineage fee must be strictly increasing in ladder length");
}
...
fn sgb_and_ssm_fee_scale_with_fragment_length() {
    ...
    let long_ssm  = fee_for(&ssm("game-semantics-with-visibility-modality-and-composition"));
...
fn sfsv_fee_scales_with_future_self_length() {
    ...
    let long  = fee_for(&sfsv("0x0102030405060708090a0b0c0d0e0f10111213141516171819"));
...
fn fixed_shape_primitives_fee_equals_base_fee() {
    ...
    let typed = mayfly();
    assert_eq!(fee_for(&typed), base_fee(&typed),
...
fn saturating_arithmetic_on_huge_fragment() {
    ...
    assert!(fee >= BASE_DEPLOY_FEE, "huge fragment must still return a sane fee");
}
