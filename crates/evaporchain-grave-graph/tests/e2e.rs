...
fn n(tag: u8) -> NodeId { NodeId([tag; 32]) }
...
fn author_social_network_full_lifecycle() {
    ...
    assert_eq!(g.footprint_of(n(ALICE)).count(), 0,
    ...
    // Alice is now Dead.
    assert!(matches!(g.node_state(&n(ALICE)), Some(NodeState::Dead { died_at_epoch: 100 })));
    ...
    g.curate_dedication(n(BOB), n(ALICE), Curation::Accepted).unwrap();
    g.curate_dedication(n(CAROL), n(ALICE), Curation::Rejected).unwrap();
    ...
    let err = g.add_edge(n(ALICE), n(EVE), EdgeKind::Living, 150).unwrap_err();
...
fn death_adds_connectivity_pre_and_post_footprint() {
    ...
    assert_eq!(pre_footprint, 0, "pre-death: no posthumous footprint exists");

    g.certify_death(n(ALICE), 100).unwrap();

    let post_footprint = g.footprint_of(n(ALICE)).count();
    assert_eq!(post_footprint, 2, "post-death: footprint = 2 dedications (Bob + Carol)");

    assert!(post_footprint > pre_footprint,
...
fn living_edge_cleared_on_death_dave_loses_alice_connection() {
    ...
    assert_eq!(g.incoming(n(DAVE)).count(), 1, "Dave has one incoming (Alice Living)");

    g.certify_death(n(ALICE), 100).unwrap();

    // After Alice's death the Living edge Alice→Dave is gone.
    assert_eq!(g.incoming(n(DAVE)).count(), 0,
        "Living edge Alice→Dave must be cleared on Alice's certified death");
    assert_eq!(g.dedications_for(n(DAVE)).count(), 0,
...
fn legacy_inverts_to_dedication_with_correct_epoch() {
    ...
    for edge in g.footprint_of(n(ALICE)) {
        assert!(
            ...
            "inverted edge must carry died_at_epoch=100, got {:?}", edge.kind
        );
    }
}
...
fn survivor_curation_independent_does_not_delete_dedication() {
    ...
    g.curate_dedication(n(BOB),   n(ALICE), Curation::Hidden).unwrap();
    g.curate_dedication(n(CAROL), n(ALICE), Curation::Accepted).unwrap();
    ...
    assert_eq!(carol_d.curation, Curation::Accepted,
...
fn dead_recipient_cannot_curate_dedication() {
    ...
    let err = g.curate_dedication(n(BOB), n(ALICE), Curation::Accepted).unwrap_err();
    assert!(matches!(err, GraveGraphError::NotRecipient),
...
fn multiple_legacy_edges_all_invert_simultaneously() {
    ...
    for target in [BOB, CAROL, DAVE, EVE] {
        g.add_edge(n(ALICE), n(target), EdgeKind::Legacy, 0).unwrap();
    }

    let inverted = g.certify_death(n(ALICE), 50).unwrap();
    assert_eq!(inverted, 4, "all 4 legacy edges must invert on one certify_death");
    assert_eq!(g.footprint_of(n(ALICE)).count(), 4,
        "footprint must include all 4 dedications");

    // Each surviving node receives exactly 1 dedication.
    for target in [BOB, CAROL, DAVE, EVE] {
        assert_eq!(g.dedications_for(n(target)).count(), 1,
...
fn direct_dedication_creation_rejected_at_all_epochs() {
    ...
    let err = g.add_edge(
        n(ALICE), n(BOB),
        EdgeKind::Dedication { died_at_epoch: 0 }, 0
    ).unwrap_err();
    assert!(matches!(err, GraveGraphError::DeadSource(_)),
...
fn non_recipient_cannot_curate_dedication() {
    ...
    let err = g.curate_dedication(n(EVE), n(ALICE), Curation::Accepted).unwrap_err();
