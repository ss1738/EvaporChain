...
use evaporchain_causal_chsh::{
    ...
    synthesize_max_cartel_samples, BlockSummary, CartelAlarm, ChshError, GateThresholds,
    GateVerdict,
};
...
fn honest_200_block_trace_satisfies_bell_bound() {
    ...
    assert!(
        honest.samples_ab.len() >= 10,
        "AB bucket must have ≥10 samples, got {}",
        honest.samples_ab.len()
    );
    assert!(
        honest.samples_ab_prime.len() >= 10,
        "AB' bucket must have ≥10 samples"
    );
    assert!(
        honest.samples_a_prime_b.len() >= 10,
        "A'B bucket must have ≥10 samples"
    );
    assert!(
        honest.samples_a_prime_b_prime.len() >= 10,
        "A'B' bucket must have ≥10 samples"
    );
    ...
    assert!(
        s < 1.8,
        "honest 200-block trace must satisfy Bell bound: got S={s:.4}"
    );
...
fn gate_passes_doctrine_thresholds_on_200_block_trace() {
    ...
    match verdict {
        GateVerdict::Pass {
            ...
            assert!(gap > 0.4, "gap must exceed min_gap=0.4; got {gap:.4}");
            assert!((s_cartel - 4.0).abs() < 1e-9, "max-cartel S must equal 4.0");
        }
...
fn compute_chsh_s_milli_is_validator_deterministic() {
    ...
    assert!(
        (0..=4_000).contains(&a),
        "S_milli must be in [0, 4000], got {a}"
    );
}
...
fn milli_s_agrees_with_float_s_within_one_milli() {
    ...
    assert!(
        (s_milli - s_float_as_milli).abs() <= 2,
...
fn rolling_alarm_doctrine_session() {
    ...
    let status = alarm
        .status()
        .expect("200 records must have triggered at least one gate run");
    ...
    assert!(
        status.s_honest < 1.8,
        "honest S must be below ceiling: {}",
        status.s_honest
    );
    assert!(
        status.s_cartel_synthetic > 2.2,
        "cartel S must exceed floor"
    );
    assert!(status.gap > 0.4, "gap must exceed min_gap");
    ...
    assert_eq!(
        status.s_cartel_synthetic_milli, 4_000,
...
fn doctrine_bell_bound_holds_for_lhv_source_classically() {
    ...
    let pairs: Vec<ConcurrentPair> = lhv_cases
        .iter()
    ...
    assert!(
        s <= 2.0 + 1e-9,
        "LHV source must satisfy Bell bound: S={s:.6}"
    );
}
...
fn adversarial_cartel_injected_as_honest_fails_gate() {
    ...
    match verdict {
        GateVerdict::Fail {
            ...
            assert!(
                reasons.iter().any(|r| r.contains("honest")),
...
fn adversarial_empty_bucket_fails_closed() {
    ...
    assert!(
        matches!(err, ChshError::EmptySample { pair_label: "AB" }),
    ...
    assert!(
        matches!(err_milli, ChshError::EmptySample { .. }),
...
fn adversarial_alarm_buffer_capped_at_capacity() {
    ...
    assert_eq!(
        alarm.records_seen(),
        400,
        "records_seen must track all calls"
    );
}
