...
fn compile_pilot() -> EvaporBytecode {
    let ast =
        parser::parse(SOURCE).unwrap_or_else(|e| panic!("VestingSchedule failed to parse: {e:?}"));
    compiler::compile(&ast).unwrap_or_else(|e| panic!("VestingSchedule failed to compile: {e:?}"))
}
