...
fn cl() -> ChainLambda { ChainLambda::new(Lambda::from_epochs(HALF_LIFE)) }

fn ns(tag: u8) -> Vec<u8> { vec![tag; 4] }
...
fn full_4_block_chain_conservation_simulation() {
    ...
    let b0 = acc;
    EnergyRedirect::new(RedirectKind::MevBurn, 50_000).apply(&mut acc).unwrap();
    ...
    let b1 = acc;
    EnergyRedirect::new(RedirectKind::Slash, 200_000).apply(&mut acc).unwrap();
    ...
    let b2 = acc;
    EnergyRedirect::new(RedirectKind::SlashSettle, 200_000).apply(&mut acc).unwrap();
    EnergyRedirect::new(RedirectKind::RefreshPayout, 30_000).apply(&mut acc).unwrap();
    ConservationCheck::redirect(&b2, &acc).unwrap();
    assert_eq!(acc.total(), 15_000_000, "B3: settle+payout must preserve total");
    ...
    assert_eq!(lambda_floor, 7_500_000, "one halving of 15M must floor to 7.5M");
...
fn mev_burn_redirect_preserves_total_and_routes_correctly() {
    ...
    let before = acc;
    EnergyRedirect::new(RedirectKind::MevBurn, 75_000).apply(&mut acc).unwrap();
...
fn slash_lifecycle_stake_to_slashed_to_refresh_pool() {
    ...
    EnergyRedirect::new(RedirectKind::Slash, 100_000).apply(&mut acc).unwrap();
    ...
    EnergyRedirect::new(RedirectKind::SlashSettle, 100_000).apply(&mut acc).unwrap();
    ConservationCheck::redirect(&step1, &acc).unwrap();
    assert_eq!(acc[Compartment::SlashedPool],       0);
    assert_eq!(acc[Compartment::RefreshPool], 100_000);
    assert_eq!(acc.total(), 500_000, "total preserved across the full slash lifecycle");
}
...
fn demurrage_redirect_accounts_to_refresh_pool() {
    ...
    let before = acc;
    EnergyRedirect::new(RedirectKind::Demurrage, 12_345).apply(&mut acc).unwrap();
    ConservationCheck::redirect(&before, &acc).unwrap();
...
fn conservation_checker_catches_energy_destruction_in_redirect() {
    ...
    assert!(matches!(err, ConservationViolation::RedirectChangedTotal { .. }),
...
fn redirect_from_empty_compartment_fails_closed_accumulator_unchanged() {
    ...
    let err = EnergyRedirect::new(RedirectKind::Slash, 100).apply(&mut acc).unwrap_err();
    use evaporchain_energy_kernel::redirect::RedirectError;
    assert!(matches!(err, RedirectError::InsufficientSource(..)),
        "insufficient source must be rejected");
    assert_eq!(acc, snapshot, "accumulator must be unchanged after a failed redirect");
}
...
fn refresh_pool_namespace_accrue_and_payout_lifecycle() {
    ...
    assert!(pool.is_empty(), "exhausted namespace must be garbage-collected");
}
...
fn refresh_pool_multi_namespace_credits_are_fully_isolated() {
    ...
    assert_eq!(pool.accrued_for(&lightcone_ns), 400_000,
        "light-cone credits must be unaffected by beacon payout");
    assert_eq!(pool.accrued_for(&beacon_ns), 0,
...
fn lambda_is_the_single_universal_decay_constant() {
    ...
    assert_eq!(energy_at_epoch(accounts, chain.half_life(), 100), accounts >> 1);
    assert_eq!(energy_at_epoch(stake,    chain.half_life(), 100), stake    >> 1);

    // Double the epochs → halve again.
    assert_eq!(energy_at_epoch(accounts, chain.half_life(), 200), accounts >> 2);
...
fn energy_at_epoch_is_monotone_decreasing_in_epochs() {
    ...
    for pair in snapshots.windows(2) {
        assert!(pair[1] <= pair[0],
            "energy_at_epoch must be non-increasing in elapsed epochs: {} > {}", pair[0], pair[1]);
    }
    // After 10 halvings: 1_000_000 >> 10 = 976 (not yet floored to 0).
    assert!(snapshots[10] < 1_000, "10 halvings must reduce energy to <0.1% of initial");
}
