...
use evaporchain_ib_validators_v2::{
    ...
    JailEntry, JailReason, JailState,
    ib_vote_v2, VoteV2, VoteV2Error,
};

// ── Helpers ───────────────────────────────────────────────────────────────

fn vid(b: u8) -> [u8; 32] { [b; 32] }
...
fn conformist_sig() -> StateSignature { prior_sig() }

fn params() -> IbParams { IbParams { lambda_mb: 100 } }
...
fn sig_for(b: u8) -> StateSignature {
    if b == 0x01 || b == 0x06 { conformist_sig() } else { high_kl_sig() }
}
...
fn commit_count(js: &JailState, epoch: u64) -> usize {
    let prior = prior_sig();
    let p = params();
    COMMITTEE.iter().filter(|(b, energy)| {
        ib_vote_v2(&sig_for(*b), &prior, &p, &vid(*b), *energy, ENERGY_FLOOR, js, epoch)
            .unwrap() == VoteV2::Commit
    }).count()
}
...
fn round1_only_unjailed_high_kl_validators_commit() {
    ...
    assert_eq!(
        ib_vote_v2(&conformist_sig(), &prior, &p, &vid(0x01), 2_000, ENERGY_FLOOR, &js, 10).unwrap(),
        VoteV2::Abstain,
        "V1 (KL=0) must abstain"
    );
    assert_eq!(
        ib_vote_v2(&high_kl_sig(), &prior, &p, &vid(0x02), 2_000, ENERGY_FLOOR, &js, 10).unwrap(),
        VoteV2::Commit,
        "V2 (high KL, unjailed) must commit"
    );
    assert_eq!(
        ib_vote_v2(&high_kl_sig(), &prior, &p, &vid(0x03), 1_500, ENERGY_FLOOR, &js, 10).unwrap(),
        VoteV2::Commit,
        "V3 (high KL, unjailed) must commit"
    );
    assert!(
        matches!(
            ib_vote_v2(&high_kl_sig(), &prior, &p, &vid(0x04), 800, ENERGY_FLOOR, &js, 10).unwrap(),
            VoteV2::Jailed { reason: JailReason::ChshFailedWindow {
    ...
    assert!(
        matches!(
            ib_vote_v2(&high_kl_sig(), &prior, &p, &vid(0x05), 50, ENERGY_FLOOR, &js, 10).unwrap(),
            VoteV2::Jailed { reason: JailReason::ChshFailedWindow { .. } }
        ),
    ...
    assert!(
        matches!(
            ib_vote_v2(&conformist_sig(), &prior, &p, &vid(0x06), 1_000, ENERGY_FLOOR, &js, 10).unwrap(),
            VoteV2::Jailed { reason: JailReason::ChshFailedWindow { .. } }
        ),
...
fn round1_commit_count_is_two() {
    ...
    let js = chsh_jail_state();
    assert_eq!(commit_count(&js, 10), 2,
...
fn round2_commit_count_still_two() {
    ...
    let js = chsh_jail_state();
    assert_eq!(commit_count(&js, 25), 2,
...
fn round3_jail_expired_v4_commits() {
    ...
    let result = ib_vote_v2(
        &high_kl_sig(), &prior_sig(), &params(),
        &vid(0x04), 800, ENERGY_FLOOR, &js, 50,
    ).unwrap();
    assert_eq!(result, VoteV2::Commit,
...
fn round3_v5_energy_jailed_after_chsh_expiry() {
    ...
    let result = ib_vote_v2(
        &high_kl_sig(), &prior_sig(), &params(),
        &vid(0x05), 50, ENERGY_FLOOR, &js, 50,
    ).unwrap();
    assert!(
        matches!(result, VoteV2::Jailed {
            reason: JailReason::EnergyBelowFloor { observed: 50, floor: 100 }
        }),
...
fn round3_v6_abstains_after_chsh_expiry() {
    ...
    let js = chsh_jail_state();
    let result = ib_vote_v2(
        &conformist_sig(), &prior_sig(), &params(),
        &vid(0x06), 1_000, ENERGY_FLOOR, &js, 50,
    ).unwrap();
    assert_eq!(result, VoteV2::Abstain,
...
fn round3_commit_count_recovers_to_three() {
    ...
    let js = chsh_jail_state();
    assert_eq!(commit_count(&js, 50), 3,
...
fn doctrine_jail_reduces_then_recovers_commit_count() {
    ...
    assert!(jailed_count < free_count,
...
fn v1_conformist_always_abstains_regardless_of_jail() {
    ...
    let result = ib_vote_v2(
        &conformist_sig(), &prior_sig(), &params(),
        &vid(0x01), 2_000, ENERGY_FLOOR, &empty_js, 0,
    ).unwrap();
    assert_eq!(result, VoteV2::Abstain,
...
fn apply_energy_jail_persists_with_deterministic_expiry() {
    ...
    assert!(js.is_jailed(&vid(0x07), 199),  "jailed at last epoch before expiry");
    assert!(!js.is_jailed(&vid(0x07), 200), "free at expiry epoch (exclusive)");
}
...
fn prune_expired_frees_whole_committee_in_bulk() {
    ...
    assert_eq!(pruned, 3, "all 3 CHSH-jailed entries must be pruned at epoch=50");
    assert_eq!(js.len(), 0, "jail state must be empty after bulk prune");

    assert_eq!(
        ib_vote_v2(&high_kl_sig(), &prior_sig(), &params(), &vid(0x04), 800, ENERGY_FLOOR, &js, 50).unwrap(),
        VoteV2::Commit,
...
fn vote_is_deterministic() {
    let js = chsh_jail_state();
    let r1 = ib_vote_v2(&high_kl_sig(), &prior_sig(), &params(), &vid(0x02), 2_000, ENERGY_FLOOR, &js, 10).unwrap();
    let r2 = ib_vote_v2(&high_kl_sig(), &prior_sig(), &params(), &vid(0x02), 2_000, ENERGY_FLOOR, &js, 10).unwrap();
...
fn adversarial_zero_energy_floor_rejected() {
    let err = ib_vote_v2(
        &high_kl_sig(), &prior_sig(), &params(),
        &vid(0x01), 1_000, 0, &JailState::new(), 0,
    ).unwrap_err();
...
fn adversarial_jail_check_precedes_energy_check() {
    ...
    let result = ib_vote_v2(
        &high_kl_sig(), &prior_sig(), &params(),
        &vid(0x05), 50, ENERGY_FLOOR, &js, 10,
    ).unwrap();
    assert!(
        matches!(result, VoteV2::Jailed { reason: JailReason::ChshFailedWindow { .. } }),
...
fn adversarial_slash_overwrites_shorter_chsh_jail() {
    ...
    js.insert(vid(0x04), JailEntry {
        reason: JailReason::Slashed { code: 1 },
        expires_at_epoch: 80,
    });

    assert!( js.is_jailed(&vid(0x04), 29), "jailed before original CHSH expiry");
    assert!( js.is_jailed(&vid(0x04), 50), "jailed after CHSH expiry but before slash expiry");
    assert!( js.is_jailed(&vid(0x04), 79), "jailed at epoch 79");
    assert!(!js.is_jailed(&vid(0x04), 80), "free at slash expiry (exclusive)");

    let result = ib_vote_v2(
        &high_kl_sig(), &prior_sig(), &params(),
        &vid(0x04), 1_000, ENERGY_FLOOR, &js, 50,
    ).unwrap();
    assert!(
        matches!(result, VoteV2::Jailed { reason: JailReason::Slashed { code: 1 } }),
...
fn adversarial_jail_expiry_boundary_is_exclusive() {
    ...
    assert!(
        matches!(
            ib_vote_v2(&high_kl_sig(), &prior_sig(), &params(), &vid(0x04), 800, ENERGY_FLOOR, &js, 49).unwrap(),
            VoteV2::Jailed { .. }
        ),
...
fn adversarial_cross_validator_jail_isolation() {
    ...
    let js = chsh_jail_state();
    assert_eq!(
        ib_vote_v2(&high_kl_sig(), &prior_sig(), &params(), &vid(0x02), 2_000, ENERGY_FLOOR, &js, 10).unwrap(),
        VoteV2::Commit,
