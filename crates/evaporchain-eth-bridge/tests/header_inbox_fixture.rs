//! Generate the cross-side header-finality test fixture.
//!
//! Produces `ethereum-bridge/contracts/fixtures/header_inbox_5.json`
//! with the same 5 deterministic BLS keypairs as the commit-cert fixture,
//! plus a hand-rolled finalised-header tuple and the corresponding
//! aggregate signature.
//!
//! Consumed by `EvaporHeaderInbox.t.sol::test_realFinalisedHeaderAccepted`.
...
#[test]
fn generate_header_inbox_5_fixture() {
    let epoch: u64 = 1;
    let height: u64 = 7_500;
    let block_hash = keccak256(b"evaporchain block 7500");
    let state_root = keccak256(b"state root @ 7500");
    let mmr_root = keccak256(b"mmr root @ 7500");
    ...
    let validators: Vec<Validator> = pks_compressed
        ...
        .collect();
    let (valset_root, total) = compute_root(epoch, &validators).unwrap();
    assert_eq!(total, 500);

    // Header message hash:
    //   keccak256(DOMAIN_TAG_HEADER || height || blockHash || stateRoot || mmrRoot || epoch)
    let dom = keccak256(DOMAIN_TAG_HEADER_TEXT.as_bytes());
    ...
    let fixture = json!({
        ...
        "expected_total_stake": total.to_string(),
        "_unused_commit_tag": DOMAIN_TAG_COMMIT_TEXT,  // documents both tags exist
    });
