...
fn four_validator_misbehavior_entropy_ordering() {
    ...
    assert_eq!(
        slash_b, STAKE_B,
        "uniform 50/50 must yield full-stake slash"
    );
    ...
    assert!(
        slash_c < STAKE_C,
        "skewed distribution must yield partial slash (not full stake)"
    );
...
fn conservation_triplet_slash_settle_refresh_pool_closed() {
    ...
    let s0 = acc;
    EnergyRedirect::new(RedirectKind::Slash, slash_b)
        .apply(&mut acc)
        .unwrap();
    ...
    assert_eq!(
        acc.total(),
        total_stake,
        "slash redirect must preserve total"
    );

    // Step 2: Slash D (Stake → SlashedPool).
    let s1 = acc;
    EnergyRedirect::new(RedirectKind::Slash, slash_d)
        .apply(&mut acc)
        .unwrap();
    ...
    assert_eq!(
        acc.total(),
        total_stake,
        "second slash must still preserve total"
    );

    // Step 3: SlashSettle — all slashed energy → RefreshPool.
    let s2 = acc;
    let settle_amount = slash_b + slash_d;
    EnergyRedirect::new(RedirectKind::SlashSettle, settle_amount)
        .apply(&mut acc)
        .unwrap();
    ConservationCheck::redirect(&s2, &acc).unwrap();
    assert_eq!(
        acc[Compartment::SlashedPool],
        0,
        "SlashedPool must be drained after settle"
    );
    assert_eq!(
        acc[Compartment::RefreshPool],
        settle_amount,
        "RefreshPool receives the settled energy"
    );
    assert_eq!(
        acc.total(),
        total_stake,
        "settle must preserve total — triplet closed"
    );
}
...
fn uniform_2_always_full_stake_slash() {
    ...
    for stake in [1, 100, 50_000, 1_000_000, u64::MAX / 2] {
        ...
        assert_eq!(
            s, stake,
            "uniform 2-way slash must equal stake for stake={stake}"
        );
    }
}
...
fn skewed_slash_between_zero_and_full() {
    ...
    let stake = 10_000;
    let distributions = [&[9, 1][..], &[99, 1][..], &[700, 300][..], &[600, 400][..]];
    for counts in distributions {
        ...
        assert!(
            s < stake,
            "skewed distribution must yield partial slash (< full stake)"
        );
    }
}
...
fn empty_counts_propagates_entropy_error() {
    ...
    assert!(
        matches!(err, EntropicSlashError::Entropy(_)),
...
fn rare_violation_higher_penalty_than_repeated_obvious_one() {
    ...
    assert_eq!(
        rare, stake,
        "rare ambiguous violation hits full-stake slash"
    );
    assert!(
        obvious < stake / 10,
...
fn multi_round_slash_settle_cycle_fully_conserved() {
    ...
    for (counts, amount) in [
        ...
        EnergyRedirect::new(RedirectKind::SlashSettle, slash)
            .apply(&mut acc)
            .unwrap();
